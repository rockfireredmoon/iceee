/*
 * Basic Firemage Script for new overworld mob Ai.
 *
...
const FIREBOLT = 240;
const PYRO_BLAST = 297;
const MELEE = 32760;	

info <- {
	name = "iceee_firemage_basic",
	enabled = true,
	author = "Emerald Icemoon",
	description = "Basic Firemage"
	speed = 2
}
...
function main() {
	if( ai.get_will() > 0 && ( randmod(5) < 4 || ai.get_might() == 0 ) ) {
		if(ai.get_will_charge() > 4 || ( ai.get_will_charge() >= 3 && randmod(5) < 4) ) 	
			ai.use_highest_once(PYRO_BLAST);
		else 	
			ai.use_highest_once(FIREBOLT);
	}
