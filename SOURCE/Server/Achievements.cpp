	...
	while (lfr.FileOpen() == true) {
		...
		if (r > 0) {
			if (strcmp(lfr.SecBuffer, "[ENTRY]") == 0) {
				...
				if(obj == NULL)
					g_Log.AddMessage("Name expected after each [OBJECTIVE]");
				else
...
void Achievement::CompleteObjective(std::string name) {
	...
	if(def == NULL)
		g_Log.AddMessageFormat("[WARNING] request to complete achievement object that does not exist. %s", name.c_str());
		...
		g_Log.AddMessageFormat("[WARNING] request to complete achievement object is already complete. %s", name.c_str());
}
