...
{
    SetNativeThreadName(GetCurrentThreadId(),threadName);
}

void SetNativeThreadName( std::thread* thread, const char* threadName)
{
    DWORD threadId = ::GetThreadId( static_cast<HANDLE>( thread->native_handle() ) );
    SetNativeThreadName(threadId,threadName);
}
...
int PrepExt_Chat(char *buffer, int characterID, const char *display_name,
...
int PrepExt_AbilityEvent(char *buffer, int creatureID, int abilityID,
		int abilityEvent) {
...
int PrepExt_Damage(char *buffer, int actorID, const char *damageString,
		const char *ability, int critical, int absorbed) {
	int wpos = 0;
	wpos += PutByte(&buffer[wpos], 4);  //_handleCreatureEventMsg
	wpos += PutShort(&buffer[wpos], 0);
...
int PrepExt_SendInfoMessage(char *buffer, const char *message,
...
int PrepExt_GenericChatMessage(char *buffer, int creatureID, const char *name,
		const char *channel, const char *message) {
...
int PrepExt_CooldownExpired(char *buffer, long actor,
		const char *cooldownCategory) {
	int wpos = 0;
	wpos += PutByte(&buffer[wpos], 4);  //_handleCreatureEventMsg
	wpos += PutShort(&buffer[wpos], 0);
...
int PrepExt_QueryResponseStringRows(char *buffer, int queryIndex,
		const STRINGLIST &strData) {
	int wpos = 0;
	wpos += PutByte(&buffer[wpos], 1);              //_handleQueryResultMsg
	wpos += PutShort(&buffer[wpos], 0);           //Placeholder for message size
	wpos += PutInteger(&buffer[wpos], queryIndex);  //Query response index
	...
	if (count > 255) {
		g_Logs.server->warn(
				"PrepExt_QueryResponseStringList too many strings: %v", count);
		count = 255;
	}
	wpos += PutShort(&buffer[wpos], count);             //Row count
...
int PrepExt_QueryResponseStringList(char *buffer, int queryIndex,
		const STRINGLIST &strData) {
	int wpos = 0;
	wpos += PutByte(&buffer[wpos], 1);              //_handleQueryResultMsg
	wpos += PutShort(&buffer[wpos], 0);           //Placeholder for message size
...
int PrepExt_QueryResponseMultiString(char *buffer, int queryIndex,
		const MULTISTRING &strData) {
	int wpos = 0;
	wpos += PutByte(&buffer[wpos], 1);              //_handleQueryResultMsg
	wpos += PutShort(&buffer[wpos], 0);           //Placeholder for message size
	wpos += PutInteger(&buffer[wpos], queryIndex);  //Query response index
	...
	for (size_t rows = 0; rows < rowCount; rows++) {
		...
		if (stringCount > 255) {
			g_Logs.server->warn(
					"PrepExt_QueryResponseMultiString too many strings: %v",
					stringCount);
			stringCount = 255;
		}
...
int PrepExt_Achievement(char *buffer, int creatureID, const char *achievement,
		const char *scoreSpec) {
	int wpos = 0;
...
int PrepExt_SendEffect(char *buffer, int sourceID, const char *effectName,
		int targetID) {
...
int PrepExt_SendHeartbeatMessage(char *buffer,
...
int PrepExt_SendAdvancedEmote(char *buffer, int creatureID,
		const char *emoteName, float emoteSpeed, int loop) {
	int wpos = 0;
...
int PrepExt_ModStopSwimFlag(char *buffer, bool swim) {
	...
	wpos += PutByte(&buffer[wpos],
			swim ? MODMESSAGE_EVENT_START_SWIM : MODMESSAGE_EVENT_STOP_SWIM); //Event code
	PutShort(&buffer[1], wpos - 3);       //Set message size
	return wpos;
}

int PrepExt_TradeCurrencyOffer(char *buffer, int offeringPlayerID,
		int tradeAmount) {
	int wpos = 0;
...
int PrepExt_CreatureEventPortalRequest(char *buffer, int actorID,
		const char *casterName, const char *locationName) {
	int wpos = 0;
	wpos += PutByte(&buffer[wpos], 4);  //_handleCreatureEventMsg
	wpos += PutShort(&buffer[wpos], 0);
...
void WriteIntegerIfNot(FILE *output, const char *label, int value,
		int ignoreVal) {
	if (value == ignoreVal)
		return;
	fprintf(output, "%s=%d\r\n", label, value);
...
int SplitEscaped(const std::string &source, const char *delim,
	...
	do {
		if(source[fpos] == '\\' && !escaped) {
			...
			if(!escaped && source.at(fpos) == *delim) {
...
int Split(const std::string &source, const char *delim,
		std::vector<std::string> &dest) {
	if (dest.size() > 0)
		dest.clear();
	size_t pos = 0;
...
int SafeFormatArg(char *destBuf, size_t maxCount, const char *format,
		va_list argList) {
	...
	if (trunc == true)
		g_Logs.server->warn("SafeFormatArg() output was truncated [%v]",
				destBuf);
...
int SafeFormatOffset(unsigned int offset, char *buffer, unsigned int bufferSize,
		const char *format, ...) {
...
int QuaternionToByteFacing(double X, double Y, double Z, double W) {
	...
	if (eulerX == 0.0) {
		if (eulerY < 0)  //From 03:01 to 6:00 (-0.0 to -1.57)
			...
			//From 12:00 to 2:59 (1.57 to 0.0)
	...
	} else   //West hemisphere
	{
		if (eulerY < 0)  //From 6:00 to 8:59 (-1.57 to -0.0)
			...
			//From 9:01 to 12:00 (0.0 to 1.57)
			eulerY += PI;
	}
...
char* FormatTime(char *outBuf, int bufSize, int seconds) {
	...
	if (Day == 0)
		Util::SafeFormat(outBuf, bufSize, "%02dh:%02dm:%02ds", Hour, Minute,
				Second);
	else
		Util::SafeFormat(outBuf, bufSize, "%02dd:%02dh:%02dm:%02ds", Day, Hour,
				Minute, Second);

	return outBuf;
}

void WriteIntegerList(FILE *output, const char* label,
...
bool CaseInsensitiveStringFind(const std::string& str1,
		const std::string& str2) {
	for (string::const_iterator c1 = str1.begin(), c2 = str2.begin();
			c1 != str1.end(); ++c1) {
		if (tolower(*c1) == tolower(*c2)) {
...
bool HasBeginning(std::string const &fullString, std::string const &beginning) {
	return fullString.size() >= beginning.size() ?
			fullString.substr(0, beginning.size()) == beginning : false;
}
...
std::string FormatTimeOfDayMS(unsigned long time) {
	time_t t = time;
...
void AssignFloatArrayFromStringSplit(float *arrayDest, size_t arraySize,
...
std::string RandomStr(unsigned int size, bool all) {
	if (all)
		...
		return RandomStrFrom(size,
				"23456789ABCDEFGHIJKLMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz");
}
