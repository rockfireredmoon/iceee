...
vector<string> ClusterEntityReader::Sections() {
	if(!mGotSections) {
		...
		for(auto a = arr.begin(); a != arr.end(); ++a) {
			if((*a).is_string()) {
				mSections.push_back((*a).as_string().substr(mCatalog.length() + mID.length() + 2));
			}
		}
...
string ClusterEntityReader::CreateSectionKey() {
	if(mSection.length() == 0)
		return StringUtil::Format("%s:%s:DEFAULT", mCatalog.c_str(), mID.c_str());
	else
		return StringUtil::Format("%s:%s:%s", mCatalog.c_str(), mID.c_str(), mSection.c_str());
}
...
vector<string> ClusterEntityReader::ListValue(const string &key, const string &separator) {
	string v = Value(key);
	if(v.length() == 0)
		return {};
	STRINGLIST l;
	...
	for(auto a = l.begin(); a != l.end() ; ++a) {
		o.push_back(StringUtil::ReplaceAll(StringUtil::ReplaceAll( (*a), StringUtil::Format("\\%s", sep.c_str()), sep), "\\\\", "\\"));
	}
	return o;
}
...
bool ClusterEntityWriter::End() {
	...
	set<string> currentKeys;
	vector<cpp_redis::reply> arr = reply.as_array();
	for(vector<cpp_redis::reply>::iterator it = arr.begin(); it != arr.end(); ++it) {
		if((*it).is_string()) {
	...
	for(auto it = mStringValues.begin(); it != mStringValues.end(); ++it) {
		if((*it).section.compare(sec) != 0) {
			if(a.size() > 0) {
				if(sec.length() > 0)
					secKey = StringUtil::Format("%s:%s:%s", mCatalog.c_str(), mID.c_str(), sec.c_str());
				else
					secKey = StringUtil::Format("%s:%s:DEFAULT", mCatalog.c_str(), mID.c_str());
		...
		a.push_back(pair<string, string>((*it).key, (*it).value));
	}
	if(a.size() > 0) {
		if(sec.length() > 0)
			secKey = StringUtil::Format("%s:%s:%s", mCatalog.c_str(), mID.c_str(), sec.c_str());
...
unsigned long Shard::GetLocalTime() {
	return mLocalTime + (mTimeSet == 0 ? 0 : g_PlatformTime.getMilliseconds() - mTimeSet);
}
unsigned long Shard::GetServerTime() {
	return mServerTime + (mTimeSet == 0 ? 0 : g_PlatformTime.getMilliseconds() - mTimeSet);
}
...
bool ClusterManager::ListAdd(const std::string &key, const std::string &value, bool sync) {
...
bool ClusterManager::ListSet(const std::string &key, const std::vector<std::string> &value, bool sync) {
	mClient.del({key});
	mClient.rpush(key, value);
	if(sync)
		mClient.sync_commit();
...
bool ClusterManager::ListRemove(const std::string &key, const std::string &value, bool sync) {
	if(sync) {
		bool ok;
...
int64_t ClusterManager::NextValue(const string &key, int incr) {
	...
	if(incr > 0) {
		...
		if(rep.ok())
			...
			g_Logs.cluster->error("Failed to increase key %v by %v. Assuming 0.", key, incr);
			return 0;
		}
	}
	else if(incr < 0) {
		...
		if(rep.ok())
			...
			g_Logs.cluster->error("Failed to decrease key %v by %v. Assuming 0.", key, incr);
			return 0;
		}
	}
	else {
		g_Logs.cluster->warn("Request to adjust key %v by %v. Assuming 0.", key, incr);
		return 0;
	}
}

int ClusterManager::Scan(const ScanCallback& task, const string &pattern, size_t max) {
	size_t cursor = 0;
	size_t count = 0;
	g_Logs.cluster->debug("Scanning for keys matching %v", pattern);
	do {
	mClient.scan(cursor, pattern, [this,task, pattern, &cursor, &count, max](const cpp_redis::reply &reply){
...
void ClusterManager::JoinedShard(unsigned long simID, int zoneID,
	...
	SYNCHRONIZED(mMutex){
		...
		if(sp.mID == 0) {
			...
			g_Logs.cluster->info("Notify cluster of new player (%v@%v)", cdata->cdef.css.display_name, zoneID);
		}
		else
			g_Logs.cluster->info("Notify cluster of player (%v@%v) update", cdata->cdef.css.display_name, zoneID);
...
void ClusterManager::ShardPing(const string &shardName, unsigned long localTime) {
	SYNCHRONIZED(mMutex){
...
void ClusterManager::OtherShardLogin(const std::string &shardName, int accountID) {
	g_Logs.cluster->info("Account %v logged in to shard %v", accountID, shardName);
}

void ClusterManager::OtherShardLogout(const std::string &shardName, int accountID) {
	g_Logs.cluster->info("Account %v logged out of shard %v", accountID, shardName);
}
...
void ClusterManager::ServerConfigurationReceived(const string &shardName,
		const string &simulatorAddress, int simulatorPort, const string &fullName, int players, unsigned long startTime, unsigned long utcTime, unsigned long localTime) {
	SYNCHRONIZED(mMutex){
		Shard s = mActiveShards[shardName];
...
void ClusterManager::SendConfiguration() {
	g_Logs.cluster->info("Received a request to send our shard configuration (Simulator %v:%v).", g_SimulatorAddress, g_SimulatorPort);
	mClient.publish(SERVER_CONFIGURATION, StringUtil::Format("%s:%s:%d:%s:%d:%lu:%lu:%lu",mShardName.c_str(), g_SimulatorAddress, g_SimulatorPort, mFullName.c_str(), GetActiveShard(mShardName).mPlayers, g_ServerLaunchTime, g_PlatformTime.getUTCMilliSeconds(), g_PlatformTime.getLocalMilliSeconds()));
	mClient.commit();
	SYNCHRONIZED(mMutex){
...
void ClusterManager::Login(int accountID) {
	if(mClient.is_connected()) {
		mClient.hset(StringUtil::Format("%s:%d", KEYPREFIX_ACCOUNT_SESSIONS.c_str(), accountID),
				StringUtil::Format("%lu", g_ServerTime),
				mShardName);
		if(mClusterable) {
			mClient.publish(LOGIN,
...
void ClusterManager::Logout(int accountID) {
	if(mClient.is_connected()) {
		string key = StringUtil::Format("%s:%d", KEYPREFIX_ACCOUNT_SESSIONS.c_str(), accountID);
...
int ClusterManager::CountAccountSessions(int accountID, bool includeLocal, bool includeRemote) {
	int count = 0;
	SYNCHRONIZED(mMutex){
...
void ClusterManager::OtherShardAuction(int auctionItemId, const string &sellerName) {
...
void ClusterManager::OtherShardAuctionItemRemoved(int auctionItemId, int auctioneerCDefID) {
	g_Scheduler.Submit([this, auctionItemId, auctioneerCDefID](){
...
void ClusterManager::OtherShardAuctionItemUpdated(int auctionItemId) {
	g_Scheduler.Submit([this, auctionItemId](){
		AuctionHouseItem item = g_AuctionHouseManager.LoadItem(auctionItemId);
		if(item.mId == 0) {
...
void ClusterManager::OtherShardChat(const string &from, const string &to, const string &channel, bool tell, int senderClanID, const string &msg) {
...
void ClusterManager::Weather(int zoneId, const string &mapName, const string &weatherType, int weight) {
	if(mClusterable) {
		g_Logs.cluster->info("Sending on weather %v (%v).", weatherType, weight);
		mClient.publish(WEATHER, StringUtil::Format("%s:%d:%s:%s:%s",mShardName.c_str(), zoneId, mapName.c_str(), weatherType.c_str(), weight));
		mClient.commit();
	}
}
...
void ClusterManager::Auction(int auctionItemId, const string &sellerName) {
	if(mClusterable) {
		...
		mClient.publish(AUCTION_ITEM, StringUtil::Format("%s:%d:%s",mShardName.c_str(), auctionItemId, sellerName.c_str()));
		mClient.commit();
	}
}
...
void ClusterManager::AuctionItemUpdated(int auctionItemId) {
	if(mClusterable) {
		g_Logs.cluster->info("Sending on updated auction item %v", auctionItemId);
		mClient.publish(AUCTION_ITEM_UPDATED, StringUtil::Format("%s:%d:%s",mShardName.c_str(), auctionItemId));
		mClient.commit();
	}
}

void ClusterManager::AuctionItemRemoved(int auctionItemId, int auctioneerCDefID) {
	if(mClusterable) {
		g_Logs.cluster->info("Sending on removed auction item %v",auctionItemId);
		mClient.publish(AUCTION_ITEM_REMOVED, StringUtil::Format("%s:%d:%d",mShardName.c_str(), auctionItemId, auctioneerCDefID));
		mClient.commit();
	}
}
...
void ClusterManager::Thunder(int zoneId, const string &mapName) {
	if(mClusterable) {
		...
		mClient.publish(WEATHER, StringUtil::Format("%s:%d:%s",mShardName.c_str(), zoneId, mapName.c_str()));
		mClient.commit();
	}
}
...
void ClusterManager::Chat(ChatMessage &message) {
	if(mClusterable) {
		g_Logs.cluster->info("Sending on chat message from %v to %v.", message.mSender, message.mChannel);
		mClient.publish(CHAT, StringUtil::Format("%s:%s:%s:%s:%s:%d:%s",mShardName.c_str(), message.mSender.c_str(), message.mRecipient.c_str(), message.mChannelName.c_str(), message.mTell ? "true" : "false", message.mSenderClanID, message.mMessage.c_str()));
		mClient.commit();
	}
}
	...
	for(auto it = keys.begin(); it != keys.end(); ++it) {
		...
		if(rep.is_array()) {
			...
			for(auto a = arr.begin() ; a != arr.end(); ++a) {
				...
				string v = (*++a).as_string();
				if( v.compare(mShardName) == 0  ) {
					g_Logs.cluster->info("Removing stale account session for %v (%v)", n, v);
					mClient.hdel(key, {n});
					mClient.commit();
				}
			}
...
void ClusterManager::RunProcessingCycle() {
	...
	if (g_ServerTime > mNextPing) {
		...
		g_Logs.cluster->debug("Sending shard ping for %v at %v", mShardName, g_ServerTime);

		mClient.publish(SERVER_PING, StringUtil::Format("%s:%lu", mShardName.c_str(), g_ServerTime));
		mClient.commit();
		...
		for (vector<string>::iterator it = r.begin(); it != r.end();
				++it) {
			ShardRemoved(*it);
		}
	}
...
void ClusterManager::Shutdown(bool wait) {
	if(mClusterable) {
		...
		try {
			...
			g_Logs.cluster->info("Failed to inform client of shutdown. %v", e.what());
		}
