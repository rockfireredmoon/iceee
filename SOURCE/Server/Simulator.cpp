...
{
	AuthHandler *authHandler = g_AuthManager.GetAuthenticator(GetByte(&readPtr[ReadPos], ReadPos));
	if(authHandler == NULL)
	{
		LogMessageL(MSG_ERROR, "Could not authentication handler");
...
//				questNutScript->CurrentQuestAct = QuestAct;
	...
	{
		wpos += pld.charPtr->questJournal.CheckQuestTalk(&SendBuf[wpos], CDef, CID, creatureInst->CreatureID);
	}
...
int SimulatorThread :: handle_query_bug_report(void)
{
	...
	Util::SafeFormat(Aux1, sizeof(Aux1), "{ \"title\": \"%s\", \"body\": \"Reported By: %s\\nCategory: %s\\n\\n%s\", \"labels\": [ \"bug\", \"in game\" ] }",
			Util::EncodeJSONString(summary).c_str(), pld.charPtr->cdef.css.display_name, query.GetString(1),
...
bool SimulatorThread :: QuestInvite(int QuestID) {
	...
	QuestDefinition *qdef = QuestDef.GetQuestDefPtrByID(QuestID);
	ActiveParty *party = creatureInst->PartyID > 0 ?  g_PartyManager.GetPartyByID(creatureInst->PartyID) : NULL;
	if(qdef != NULL) {
