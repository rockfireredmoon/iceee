	...
	if (query.name.compare("unstick") == 0)
		...
		if (creatureInst->CurrentTarget.targ != NULL
				&& CheckPermissionSimple(Perm_Account, Permission_Debug)
						== true) {
			int dist = ActiveInstance::GetPlaneRange(creatureInst,
					creatureInst->CurrentTarget.targ, 99999);
			sprintf(Aux1,
					"Name: %s (%d), ID: %d, CDef: %d (%d) (D:%d) PVPable: %s",
					creatureInst->CurrentTarget.targ->css.display_name,
					creatureInst->CurrentTarget.targ->css.level,
					creatureInst->CurrentTarget.targ->CreatureID,
					creatureInst->CurrentTarget.targ->CreatureDefID,
					creatureInst->CurrentTarget.targ->css.health, dist,
					creatureInst->CanPVPTarget(
							creatureInst->CurrentTarget.targ) ? "yes" : "no");
...
int SimulatorThread::handle_query_item_move(void) {
	...
	if (destContainer == -1) {
		g_Logs.simulator->error(
				...
				InternalID, query.args[0].c_str(), CCSID);
		return PrepExt_QueryResponseError(SendBuf, query.ID,
	...
	TradeTransaction *tradeData =
			creatureInst->actInst->tradesys.GetExistingTransaction(tradeID);
	if (tradeData != NULL) {
		creatureInst->actInst->tradesys.CancelTransaction(selfID, tradeID,
				SendBuf);
		return PrepExt_QueryResponseError(SendBuf, query.ID,
	...
	if (destContainer == EQ_CONTAINER) {
		...
		int res = destInv->VerifyEquipItem(sourceItem, destSlot,
				creatureInst->css.level, creatureInst->css.profession);
		if (res != InventoryManager::EQ_ERROR_NONE)
			return PrepExt_QueryResponseError(SendBuf, query.ID,
					InventoryManager::GetEqErrorString(res));
	}

	int mpos = origInv->ItemMove(Aux1, Aux3, &creatureInst->css,
...
int SimulatorThread::handle_query_item_delete(void) {
	...
	if (itemDef != NULL) {
		g_Logs.simulator->info("[%v] Deleting item %v [%v]", InternalID, itemID,
		...
		if (itemDef->mActionAbilityId > 0) {
			creatureInst->RemoveBuffsFromAbility(itemDef->mActionAbilityId,
					true);
		}
	}
	...
	int wpos = 0;
	wpos += RemoveItemUpdate(&SendBuf[wpos], Aux3,
...
int SimulatorThread::handle_command_pose(void) {
	...
	int wpos = 0;
	if (query.argCount == 1)
		wpos = PrepExt_GenericChatMessage(SendBuf, pld.CreatureID,
				pld.charPtr->cdef.css.display_name, "emote",
				query.args[0].c_str());
	else {
		wpos = PrepExt_SendAdvancedEmote(SendBuf, creatureInst->CreatureID,
				emoteName, emoteSpeed, loop);
		if (query.argCount >= 3 && loop == 0)
			wpos += PrepExt_SendEmoteControl(&SendBuf[wpos],
					creatureInst->CreatureID, 1);
	}
...
int SimulatorThread::handle_command_pose2(void) {
	...
	int wpos = 0;
	if (query.argCount == 1)
		wpos = PrepExt_GenericChatMessage(SendBuf, targetID,
				pld.charPtr->cdef.css.display_name, "emote",
				query.args[0].c_str());
	else {
		wpos = PrepExt_SendAdvancedEmote(SendBuf, targetID, emoteName,
				emoteSpeed, loop);
		if (query.argCount >= 3 && loop == 0)
...
int SimulatorThread::handle_command_esay(void) {
	if (query.argCount == 0) {
		SendInfoMessage("Usage: /esay Emote \"optional local say message\"",
				INFOMSG_INFO);
	} else {
		const char *emoteName = query.GetString(0);
		...
		if (sayText != NULL)
			wpos += PrepExt_GenericChatMessage(&SendBuf[wpos], pld.CreatureID,
	...
	if (abID > 0) {
		if (query.argCount == 2) {
			...
			if (ground > 0) {
				...
				if (ptr == NULL)
					ptr = creatureInst;
				creatureInst->ab[0].SetPosition(ptr->CurrentX, ptr->CurrentY,
						ptr->CurrentZ);
			}
			...
			creatureInst->ab[0].SetPosition(creatureInst->CurrentX + xoffset,
					creatureInst->CurrentY, creatureInst->CurrentZ + zoffset);
		}
...
int SimulatorThread::handle_command_partylowest(void) {
	...
	if (party != NULL) {
		...
		if (newTarget != oldTarget) {
			...
			int wpos = PrepExt_ChangeTarget(SendBuf, pld.CreatureID,
...
int SimulatorThread::handle_command_who(void) {
	...
	for (it = Simulator.begin(); it != Simulator.end(); ++it) {
		if (it->isConnected == true && it->ProtocolState == 1
				&& it->LoadStage == LOADSTAGE_GAMEPLAY) {
			...
			if (cd != NULL && zd != NULL) {
				...
				WritePos += PrepExt_SendInfoMessage(&SendBuf[WritePos], Aux1,
						INFOMSG_INFO);
...
int SimulatorThread::handle_command_gmwho(void) {
	...
	for (it = Simulator.begin(); it != Simulator.end(); ++it) {
		if (it->isConnected == true && it->ProtocolState == 1
				&& it->LoadStage == LOADSTAGE_GAMEPLAY) {
			CharacterData *cd = it->pld.charPtr;
			Util::SafeFormat(Aux1, sizeof(Aux1), "%s (%s, AID:%d)",
...
int SimulatorThread::handle_command_chwho(void) {
	...
	STRINGLIST results;
	g_ChatChannelManager.EnumPlayersInMemberChannels(InternalID, channelName,
			results);
	int wpos = 0;
	for (size_t i = 0; i < results.size(); i++) {
		wpos += PrepExt_SendInfoMessage(&SendBuf[wpos], results[i].c_str(),
				INFOMSG_INFO);
	...
	if (results.size() == 0) {
		...
		if (channelName != NULL) {
			Util::SafeFormat(Aux3, sizeof(Aux3), "You are not in channel [%s].",
					channelName);
			errorMsg = Aux3;
		}
		wpos += PrepExt_SendInfoMessage(&SendBuf[wpos], errorMsg,
				INFOMSG_ERROR);
	}
	if (wpos > 0)
		AttemptSend(SendBuf, wpos);
...
int SimulatorThread::handle_command_give(void) {
	...
	if (item->mID == 0) {
		Util::SafeFormat(Aux1, sizeof(Aux1), "Item name not found [%s]",
				itemName);
...
int SimulatorThread::handle_command_giveall(void) {
	...
	for (int i = 0; i < found; i++) {
		...
		if (slot >= 0) {
			...
			g_Logs.simulator->debug("[%v] No more free slots.", InternalID);
			break;
		}
	}
...
int SimulatorThread::handle_command_giveapp(void) {
	...
	if (query.argCount < 2)
		return PrepExt_QueryResponseError(SendBuf, query.ID,
				"Usage: /giveapp equipType appString");
...
int SimulatorThread::handle_command_deleteall(void) {
	...
	WritePos = 0;
	for (a = 0; a < size; a++) {
		WritePos += RemoveItemUpdate(&SendBuf[WritePos], Aux1,
...
void SimulatorThread::handle_command_deleteabove(void) {
	...
	while (bFound == true) {
		...
		for (size_t a = 0;
			...
			if (slot >= delSlot) {
				WritePos +=
						RemoveItemUpdate(&SendBuf[WritePos], Aux3,
								&pld.charPtr->inventory.containerList[INV_CONTAINER][a]);
...
int SimulatorThread::handle_query_trade_shop(void) {
	...
	EssenceShop *esptr = creatureInst->actInst->itemShopList.GetEssenceShopPtr(
			CDef, Aux1, &iptr);
	...
	if (newItemID == itemPtr->mID) {
		...
		if (g_ItemManager.CheckVirtualItemGamble(itemPtr,
				creatureInst->css.level, &viParam, cost) == true) {
...
int SimulatorThread::handle_query_trade_essence(void) {
	...
	EssenceShopItem *iptr = NULL;
	EssenceShop *esptr =
			creatureInst->actInst->essenceShopList.GetEssenceShopPtr(CDef, Aux1,
					&iptr);
	if (esptr == NULL || iptr == NULL) {
		g_Logs.simulator->error(
	...
	if (currentItemCount < iptr->EssenceCost) {
		g_Logs.simulator->warn("[%v] Essence requirement for item %v: %v / %v",
				InternalID, esptr->EssenceID, currentItemCount,
				iptr->EssenceCost);
		return PrepExt_QueryResponseString(SendBuf, query.ID,
...
int SimulatorThread::handle_command_grove(void) {
	...
	if (groveCount > 0) {
		...
		wpos += PrepExt_SendInfoMessage(&SendBuf[wpos], "No groves found.",
				INFOMSG_INFO);
	}
	wpos += PrepExt_QueryResponseString(&SendBuf[wpos], query.ID, "OK");
	return wpos;
...
int SimulatorThread::handle_command_pvp(void) {
	...
	if (zoneCount > 0) {
		...
		wpos += PrepExt_SendInfoMessage(&SendBuf[wpos], "No arenas found.",
				INFOMSG_INFO);
	}
	wpos += PrepExt_QueryResponseString(&SendBuf[wpos], query.ID, "OK");
	return wpos;
...
int SimulatorThread::handle_query_quest_genericdata(void) {
	...
	g_Logs.simulator->trace("[%v]   Requested quest.genericdata for %v",
			InternalID, QID);
	return pld.charPtr->questJournal.QuestGenericData(SendBuf, sizeof(SendBuf),
			Aux3, QID, query.ID);
}
...
int SimulatorThread::handle_query_quest_join(void) {
	...
	if (ActiveInstance::GetPlaneRange(creatureInst, giverNPC, INTERACT_RANGE)
			> INTERACT_RANGE)
	...
	if (qdef->QuestGiverID != giverNPC->CreatureDefID)
		return ErrorMessageAndQueryOK(SendBuf,
	...
	if (qdef->accountQuest) {
		...
		AccountData *acc = g_AccountManager.GetActiveAccountByID(
				creatureInst->charPtr->AccountID);
...
int SimulatorThread::handle_query_quest_getcompletequest(void) {
	...
	return pld.charPtr->questJournal.QuestGetCompleteQuest(SendBuf, Aux3, CDef,
			query.ID);
}
...
int SimulatorThread::handle_query_quest_complete(void) {
	...
	if (CID == creatureInst->CreatureID) {
		...
		if (ActiveInstance::GetBoxRange(creatureInst, questEnder)
				> INTERACT_RANGE)
			return ErrorMessageAndQueryOK(SendBuf,
...
//	QuestReference *qr = g_Q
	...
	response = creatureInst->ProcessQuestRewards(QID, qr->Outcome,
			rewardedItems);
...
int SimulatorThread::handle_query_quest_hack(void) {
	...
	if (creature == NULL) {
		...
		int wpos = creature->charPtr->questJournal.ForceComplete(
				creature->CreatureID, qdef->questID, &Aux1[0]);
...
int SimulatorThread::handle_command_complete(void) {
	...
	int wpos = pld.charPtr->questJournal.ForceAllComplete(
			creatureInst->CreatureID, SendBuf);
	wpos += PrepExt_QueryResponseString(&SendBuf[wpos], query.ID, "OK");
	return wpos;
}
...
int SimulatorThread::handle_query_trade_start(void) {
	...
	if (rval <= 0) {
		rval = PrepExt_SendInfoMessage(SendBuf, GetErrorString(rval),
				INFOMSG_ERROR);
...
int SimulatorThread::protected_helper_query_trade_accept(void) {
	...
	if (tradeData->MutualAccept() == true) {
		...
		for (size_t a = 0; a < tradeData->player[0].itemList.size(); a++) {
			...
			if (item == NULL) {
				...
				wpos += p1->inventory.RemoveItemUpdate(&SendBuf[wpos], Aux3,
						item);
		...
		for (size_t a = 0; a < tradeData->player[1].itemList.size(); a++) {
			...
			if (item == NULL) {
				...
				wpos += p2->inventory.RemoveItemUpdate(&SendBuf[wpos], Aux3,
						item);
	...
	exit: return PrepExt_QueryResponseString(SendBuf, query.ID, "OK");
}
...
int SimulatorThread::handle_command_refashion(void) {
	...
	if (query.argCount < 1)
		error = true;
	else if (query.args[0].compare(Aux3) != 0)
		error = true;
	...
	if (error == true) {
		...
		if (itemPtr1 != NULL && itemPtr2 != NULL) {
			...
			SendInfoMessage(Aux3, INFOMSG_INFO);
			Util::SafeFormat(Aux3, sizeof(Aux3), "%s will then be destroyed!",
					itemPtr2->mDisplayName.c_str());
...
int SimulatorThread::handle_command_backup(void) {
	...
	int wpos = 0;
	wpos +=
			PrepExt_SendInfoMessage(&SendBuf[wpos],
...
int SimulatorThread::handle_command_restore(void) {
	...
	if (pld.charPtr->originalAppearance.size() == 0)
		...
		creatureInst->css.SetAppearance(
				pld.charPtr->originalAppearance.c_str());

		wpos += PrepExt_SendInfoMessage(&SendBuf[wpos], "Appearance restored.",
				INFOMSG_INFO);
...
int SimulatorThread::handle_command_setstat(void) {
	...
	if (query.argCount < 2) {
		SendInfoMessage(
				"Usage: select target and use /setstat statName newValue",
				INFOMSG_ERROR);
		return PrepExt_QueryResponseString(SendBuf, query.ID, "OK");
	}
...
int SimulatorThread::handle_command_adjustexp(void) {
	...
	if (CheckPermissionSimple(Perm_Account, Permission_Admin) == false
			&& CheckPermissionSimple(Perm_Account, Permission_Builder) == false)
		return PrepExt_QueryResponseError(SendBuf, query.ID,
				"Permission denied.");
...
int SimulatorThread::handle_command_ccc(void) {
	...
	int wpos = 0;
	if (category != NULL)
		wpos = PrepExt_CooldownExpired(SendBuf, creatureInst->CreatureID,
				category);
	else {
		STRINGLIST categoryList;
		g_AbilityManager.GetCooldownCategoryStrings(categoryList);
		for (size_t i = 0; i < categoryList.size(); i++) {
			wpos += PrepExt_CooldownExpired(&SendBuf[wpos],
					creatureInst->CreatureID, categoryList[i].c_str());
			CheckWriteFlush(wpos);
		}
	}
...
int SimulatorThread::handle_command_ban(void) {
	...
	if (query.argCount < 2)
		return PrepExt_QueryResponseError(SendBuf, query.ID,
				"Usage: /ban \"Character Name\" duration");
	...
	for (it = Simulator.begin(); it != Simulator.end(); ++it) {
		if (it->pld.charPtr == banChar) {
			Util::SafeFormat(Aux1, sizeof(Aux1), "%s [%s] has been suspended.",
					charName, it->pld.accPtr->Name);
...
int SimulatorThread::handle_command_setbuildpermission(void) {
	...
	if (query.argCount != 2 && query.argCount != 6)
		return PrepExt_QueryResponseError(SendBuf, query.ID,
				"Usage: /setbuildpermission name instance [x1,y1,x2,y2]");
...
int SimulatorThread::handle_command_setpermission(void) {
	...
	if (query.argCount < 3)
		return PrepExt_QueryResponseError(SendBuf, query.ID,
				"Usage: /setpermission name value permission");
...
int SimulatorThread::handle_command_setpermissionc(void) {
	...
	if (query.argCount < 3)
		return PrepExt_QueryResponseError(SendBuf, query.ID,
				"Usage: /setpermissionc name value permission");
...
int SimulatorThread::handle_command_setbehavior(void) {
	...
	sprintf(Aux1, "BehaviorFlag is now: %lu",
...
//		tag->mSlotIndex = slot;
		...
		if (receivingCreature->CreatureID == robin->mCreaturePtr->CreatureID) {
			g_Logs.simulator->info(
					"[%d] Sending Offer Loot Round Robin to looter, so returning with this response",
					InternalID);
			return WriteIdx;
		} else {
			g_Logs.simulator->info(
					"[%d] Sending Offer Loot Round Robin to someone other than looter, so returning on its simulator",
					InternalID);
...
//			WritePos = PrepExt_ItemDef(SendBuf, item, ProtocolState);
	...
	if (mode == LOOT_MASTER) {
		...
		if (receivingCreature->CreatureID == leader->mCreaturePtr->CreatureID) {
			g_Logs.simulator->info(
					"[%d] Sending Offer Loot Master to looter, so returning with this response",
					InternalID);
			return WriteIdx;
		} else {
			g_Logs.simulator->info(
					"[%d] Sending Offer Loot Master to someone other than looter, so returning on its simulator",
					InternalID);
...
int SimulatorThread::handle_query_mod_igforum_openthread(void) {
	...
	unsigned int wpos = PrepExt_QueryResponseMultiString(SendBuf, query.ID,
			results);
	...
	if (wpos >= sizeof(SendBuf) - 1)
		...
		g_Logs.simulator->warn(
				"[%d] IGF openthread response dangerous size: %d", InternalID,
				wpos);
...
int SimulatorThread::handle_command_dtrig(void) {
	...
	case 15:
		if (query.argCount >= 2) {
			...
			for (size_t i = 0; i < creatureInst->actInst->NPCListPtr.size();
					i++) {
	...
	case 17: {
		...
		g_SceneryManager.EnumPropsInRange(pld.CurrentZoneID,
				creatureInst->CurrentX, creatureInst->CurrentZ, searchlen,
				search);
	...
	case 900: {
		...
		for (int y = y1; y <= y2; y++) {
			for (int x = x1; x <= x2; x++) {
				sprintf(Aux1, "Scenery\\%d\\x%03dy%03d.txt", pld.zoneDef->mID,
						x, y);
	...
	case 1000: {
		...
		creatureInst->actInst->SetAllPlayerPVPStatus(creatureInst->CurrentX,
				creatureInst->CurrentZ, range, state);
	}
		break;
	}
	return PrepExt_QueryResponseString(SendBuf, query.ID, "OK");
...
int SimulatorThread::handle_command_set_earsize(void) {
	...
	creature->actInst->LSendToLocalSimulator(SendBuf, wpos, creature->CurrentX,
			creature->CurrentZ);
	return PrepExt_QueryResponseString(SendBuf, query.ID, "OK");
}
...
int SimulatorThread::handle_command_sdiag(void) {
	...
	Util::SafeFormat(Aux1, sizeof(Aux1), "Self diagnostic check is %s",
			((val != 0) ? "ON" : "OFF"));
	...
	if (query.argCount == 0) {
		...
		if (pld.DebugPingServerTotalReceived != 0)
			average = pld.DebugPingServerTotalTime
					/ pld.DebugPingServerTotalReceived;
		sprintf(Aux1, "Pings:%d  Lowest:%d  Highest:%d  Avg:%d  Sync:%d/%d",
...
int SimulatorThread::handle_command_info(void) {
	...
	if (query.argCount >= 1) {
		...
		if (query.args[0].compare("scenerytile") == 0)
			sprintf(Aux1, "Scenery Tile: %d, %d",
					creatureInst->CurrentX / pld.zoneDef->mPageSize,
					creatureInst->CurrentZ / pld.zoneDef->mPageSize);
		if (Aux1[0] != 0)
			SendInfoMessage(Aux1, INFOMSG_INFO);
	}
	return PrepExt_QueryResponseString(SendBuf, query.ID, "OK");
...
int SimulatorThread::handle_command_pathlinks(void) {
	...
	g_SceneryManager.EnumPropsInRange(pld.CurrentZoneID, creatureInst->CurrentX,
			creatureInst->CurrentZ, radius, results);
	int propChanges = 0;
	for (size_t i = 0; i < results.size(); i++) {
...
int SimulatorThread::handle_command_partyinvite(void) {
	...
	if (query.argCount < 1)
		return PrepExt_QueryResponseError(SendBuf, query.ID,
				"Usage: /partyinvite \"Character Name\"");
...
int SimulatorThread::handle_command_roll(void) {
	...
	if (creatureInst->PartyID == 0 || party == NULL) {
		...
		int wpos = 0;
		for (size_t mi = 0; mi < party->mMemberList.size(); mi++) {
			Util::SafeFormat(Aux1, sizeof(Aux1), "%s rolled %d",
