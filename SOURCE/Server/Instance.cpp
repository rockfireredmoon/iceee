			...
			if (strcmp(lfr.SecBuffer, "[ENTRY]") == 0) {
				...
				if (opIndex >= 0) {
					if (strcmp(lfr.SecBuffer, "NAME") == 0)
						...
						mMapList[opIndex].parentMapImage = lfr.BlockToStringC(1,
								0);
					else if (strcmp(lfr.SecBuffer, "NUMPAGESACROSS") == 0)
						mMapList[opIndex].numPagesAcross =
								(float) lfr.BlockToDblC(1);
					else if (strcmp(lfr.SecBuffer, "NUMPAGESDOWN") == 0)
						mMapList[opIndex].numPagesDown =
								(float) lfr.BlockToDblC(1);
						...
						mMapList[opIndex].scale_width = (float) lfr.BlockToDblC(
								1);
					else if (strcmp(lfr.SecBuffer, "SCALE_HEIGHT") == 0)
						mMapList[opIndex].scale_height =
								(float) lfr.BlockToDblC(1);
					else
						g_Logs.data->error(
					...
					lfr.CloseCurrent();
					g_Logs.data->error(
							"Error reading %v, data must begin with an [ENTRY] block.",
							fileName);
					return -1;
				}
			}
...
int MapLocationHandler::SearchLocation(int zone, int x, int z) {
	...
	if (found >= 0) {
		...
		for (a = 0; a < mLocationSet[found].mLocationList.size(); a++)
			if (x >= mLocationSet[found].mLocationList[a].x1
					&& x <= mLocationSet[found].mLocationList[a].x2)
				if (z >= mLocationSet[found].mLocationList[a].y1
						&& z <= mLocationSet[found].mLocationList[a].y2)
...
const char* MapLocationHandler::GetInternalMapName(int zone, int x, int z) {
	...
	if (found >= 0) {
		...
		for (i = 0; i < mLocationSet[found].mLocationList.size(); i++)
			if (x >= mLocationSet[found].mLocationList[i].x1
					&& x <= mLocationSet[found].mLocationList[i].x2)
				if (z >= mLocationSet[found].mLocationList[i].y1
						&& z <= mLocationSet[found].mLocationList[i].y2)
...
int MapLocationHandler::ResolveItems(void) {
	...
	for (a = 0; a < mLocationSet.size(); a++) {
		for (b = 0; b < mLocationSet[a].mLocationList.size(); b++) {
			...
			mld->resolvedIndex = MapDef.GetIndexByName(mld->MapName.c_str(),
					"Region");
			if (mld->resolvedIndex == -1) {
				g_Logs.data->warn("Could not resolve location name [%v]",
						mld->MapName.c_str());
...
void ActiveInstance::Clear(void) {
	...
	for (a = 0; a < NPCListPtr.size(); a++) {
		...
		if (pendingOperations.Debug_HasCreature(NPCListPtr[a]) == true) {
			g_Logs.server->error(
					...
					NPCListPtr[a]->css.display_name, NPCListPtr[a]);
		}
	...
	for (a = 0; a < SidekickListPtr.size(); a++) {
		...
		if (pendingOperations.Debug_HasCreature(SidekickListPtr[a]) == true)
			g_Logs.server->error(
					"Creature not removed from access list: %v (%v)",
					SidekickListPtr[a]->css.display_name, SidekickListPtr[a]);
...
PVP::PVPGame * ActiveInstance::StartPVP(int type) {
	if (pvpGame != NULL) {
		g_Logs.server->error("Already in PVP game %v for %v", pvpGame->mId,
				mZone);
		return NULL;
	}
...
CreatureInstance * ActiveInstance::GetPlayerByID(int id,
		list<CreatureInstance>::iterator &resIterator) {
	list<CreatureInstance>::iterator it;
	for (it = PlayerList.begin(); it != PlayerList.end(); ++it) {
		if (it->CreatureID == id) {
			resIterator = it;
...
CreatureInstance * ActiveInstance::LoadPlayer(CreatureInstance *source,
...
int ActiveInstance::RemovePlayerByID(int creatureID) {
	...
	for (it = PlayerList.begin(); it != PlayerList.end(); ++it) {
		if (it->CreatureID == creatureID) {
			...
			g_Logs.server->info("Player removed (%v) at (%v)",
					it->css.display_name, &*it);
			PlayerList.erase(it);
			RebuildPlayerList();
...
void ActiveInstance::SetEnvironment(std::string environment) {
	if (environment.compare(mEnvironment) != 0) {
		...
		int wpos = PrepExt_SendEnvironmentUpdateMsg(GSendBuf, this,
				mZoneDefPtr->mName.c_str(), mZoneDefPtr, -1, -1, 0);
...
int ActiveInstance::LSendToAllSimulator(const char *buffer, int length,
		int ignoreIndex) {
	int success = 0;
	for (size_t i = 0; i < RegSim.size(); i++) {
		if ((RegSim[i]->CheckStateGameplayProtocol() == true)
...
void ActiveInstance::LSendToLocalSimulator(const char *buffer, int length,
		int x, int z, int ignoreIndex) {
...
int ActiveInstance::LSendToOneSimulator(const char *buffer, int length,
		int simIndex) {
...
void ActiveInstance::LSendToOneSimulator(const char *buffer, int length,
		SimulatorThread *simPtr) {
	if (simPtr == NULL)
		return;
...
void ActiveInstance::SendActors(void) {
	...
	if (g_ServerTime >= mNextCreatureBroadcast) {
		...
		for (b = 0; b < RegSim.size(); b++) {
			...
			for (a = 0; a < NPCListPtr.size(); a++) {
				if (abs(
						NPCListPtr[a]->CurrentX
								- RegSim[b]->creatureInst->CurrentX)
						> g_CreatureListenRange)
					continue;
				if (abs(
						NPCListPtr[a]->CurrentZ
								- RegSim[b]->creatureInst->CurrentZ)
						> g_CreatureListenRange)
					continue;
			...
			for (a = 0; a < SidekickListPtr.size(); a++) {
				if (abs(
						SidekickListPtr[a]->CurrentX
								- RegSim[b]->creatureInst->CurrentX)
						> g_CreatureListenRange)
					continue;
				if (abs(
						SidekickListPtr[a]->CurrentZ
								- RegSim[b]->creatureInst->CurrentZ)
						> g_CreatureListenRange)
					continue;
	...
	if (g_ServerTime >= mNextCreatureLocalScan) {
		...
		for (size_t p = 0; p < RegSim.size(); p++) {
			...
			for (size_t c = 0; c < NPCListPtr.size(); c++) {
				if (abs(
						NPCListPtr[c]->CurrentX
								- RegSim[p]->creatureInst->CurrentX)
						> g_LocalActivityRange)
					continue;
				if (abs(
						NPCListPtr[c]->CurrentZ
								- RegSim[p]->creatureInst->CurrentZ)
						> g_LocalActivityRange)
					continue;
		...
		for (size_t i = 0; i < localActive.size(); i++) {
			NPCListPtr[i]->SetServerFlag(ServerFlags::LocalActive,
					(localActive[i] > 0));
		}
#endif
	}
		...
		if (NPCListPtr[i]->serverFlags & ServerFlags::TriggerDelete) {
			...
			if (g_ServerTime >= NPCListPtr[i]->deathTime + compareTime) {
				if (NPCListPtr[i]->spawnTile != NULL)
					NPCListPtr[i]->spawnTile->RemoveAttachedCreature(
							NPCListPtr[i]->CreatureID);
				deadList.push_back(NPCListPtr[i]->CreatureID);
			}
		}
	}
...
#ifdef CREATUREQAV
	for (a = 0; a < NPCListPtr.size(); a++) {
		if (orig->_ValidTargetFlag(NPCListPtr[a], TargetStatus::Enemy_Alive)
				== true) {
...
#endif
	for (a = 0; a < SidekickListPtr.size(); a++) {
		if (orig->_ValidTargetFlag(SidekickListPtr[a],
...
CreatureInstance * ActiveInstance::GetClosestAggroTarget(
		CreatureInstance *sourceActor) {
	...
	for (size_t i = 0; i < PlayerListPtr.size(); i++) {
		...
		if (PlayerListPtr[i]->Faction == sourceActor->Faction)
			continue;
		if (abs(PlayerListPtr[i]->CurrentY - sourceActor->CurrentY)
				>= AGGRO_ELEV_RANGE)
			continue;
	...
	for (size_t i = 0; i < NPCListPtr.size(); i++) {
		...
		if (NPCListPtr[i]->Faction == sourceActor->Faction)
			continue;
		if (abs(NPCListPtr[i]->CurrentY - sourceActor->CurrentY)
				>= AGGRO_ELEV_RANGE)
			continue;
...
int ActiveInstance::GetBoxRange(CreatureInstance *obj1,
		CreatureInstance *obj2) {
...
int ActiveInstance::GetPlaneRange(CreatureInstance *obj1,
		CreatureInstance *obj2, int threshold) {
...
int ActiveInstance::GetActualRange(CreatureInstance *obj1,
		CreatureInstance *obj2, int threshold) {
...
int ActiveInstance::GetPointRange(CreatureInstance *obj1, float x, float y,
		float z, int threshold) {
...
int ActiveInstance::GetPointRangeXZ(CreatureInstance *obj1, float x, float z,
		int threshold) {
	//Returns distance between a creature instance and an arbitrary point.
	int xlen = abs(obj1->CurrentX - (int) x);
	if (xlen > threshold)
...
CreatureInstance * ActiveInstance::GetInstanceByCID(int CID) {
	if (PlayerListPtr.size() != PlayerList.size()) {
		g_Logs.server->error("GetInstanceByCID PlayerList Mismatch %v:%v",
				PlayerListPtr.size() != PlayerList.size());
		return NULL;
	}
	...
	if (SidekickListPtr.size() != SidekickList.size()) {
		g_Logs.server->error("GetInstanceByCID PlayerList Mismatch %v:%v",
				SidekickListPtr.size() != SidekickList.size());
		return NULL;
	}
	for (a = 0; a < SidekickListPtr.size(); a++) {
...
void ActiveInstance::ResolveCreatureDef(int CreatureInstanceID,
		int *responsePtr) {
#ifndef CREATUREMAP
	size_t a;
	for(a = 0; a < NPCListPtr.size(); a++)
...
CreatureInstance* ActiveInstance::SpawnCreate(CreatureInstance * sourceActor,
		int CDefID) {
	...
	retPtr = &NPCList.back();
#else
	CREATURE_IT it = NPCList.insert(NPCList.end(),
			CREATURE_PAIR(newItem.CreatureID, newItem));
...
CreatureInstance* ActiveInstance::SpawnGeneric(int CDefID, int x, int y, int z,
		int facing, int SpawnFlags) {
	...
	if (retPtr != NULL) {
		...
		int aggro = 0;
		if ((SpawnFlags & SpawnPackageDef::FLAG_USABLE)
		...
		if (aggro == 1
				&& (cdef->css.ai_package[0] == 0
						&& retPtr->css.ai_package[0] == 0))
			aggro = 0;
...
CreatureInstance* ActiveInstance::SpawnAtProp(int CDefID, int PropID,
		int duration, int elevationOffset) {
	CreatureDefinition *cdef = CreatureDef.GetPointerByCDef(CDefID);
	if (cdef == NULL) {
		g_Logs.server->error("SpawnAtProp() creature def not found: %v",
				CDefID);
		return NULL;
	}
	...
	int facing = Util::QuaternionToByteFacing(so->QuatX, so->QuatY, so->QuatZ,
			so->QuatW);
...
void ActiveInstance::EraseAllCreatureReference(CreatureInstance *object) {
	...
	if (pendingOperations.Debug_HasCreature(object)) {
		g_Logs.server->warn(
				"EraseAllCreatureReference still has pending object for erased creature: %v, %v",
				object->CreatureID, object->css.display_name);
		pendingOperations.UpdateList_Remove(object);
		pendingOperations.DeathList_Remove(object);
	};
}
...
void ActiveInstance::RunDeath(CreatureInstance *object) {
	if (nutScriptPlayer != NULL) {
		...
		if (it != PlayerListPtr.end()) {
			...
			QuestScript::QuestNutPlayer *nut = GetSimulatorQuestNutScript(
					object->simulatorPtr);
			if (nut != NULL) {
...
int ActiveInstance::EraseIndividualReference(CreatureInstance *object) {
	...
	if (PlayerList.size() != PlayerListPtr.size())
		g_Logs.server->error("PlayerList mismatch: %v, %v", PlayerList.size(),
				PlayerListPtr.size());
	if (NPCList.size() != NPCListPtr.size())
		g_Logs.server->error("NPCList mismatch: %v, %v", NPCList.size(),
				NPCListPtr.size());
	if (SidekickList.size() != SidekickListPtr.size())
		g_Logs.server->error("SidekickList mismatch: %v, %v",
				SidekickList.size(), SidekickListPtr.size());
...
int ActiveInstance::RemoveNPCInstance(int CreatureID) {
	...
	for (it = NPCList.begin(); it != NPCList.end(); ++it) {
		...
		if (ptr->CreatureID == CreatureID) {
			...
			LSendToLocalSimulator(GSendBuf, size, ptr->CurrentX, ptr->CurrentZ,
					-1);
...
void ActiveInstance::UnHate(int CreatureDefID) {
	for (size_t i = 0; i < PlayerListPtr.size(); i++)
		if (PlayerListPtr[i]->CurrentTarget.hasTargetCDef(CreatureDefID)
				&& PlayerListPtr[i]->HasStatus(StatusEffects::PVPABLE))
...
ActiveInstance * ActiveInstanceManager::GetPtrByZoneOwner(int zoneID,
		int ownerDefID) {
...
ActiveInstance * ActiveInstanceManager::GetPtrByZoneInstanceID(int zoneID,
		int instanceID) {
...
ActiveInstance * ActiveInstanceManager::GetPtrByZonePartyID(int zoneID,
		int partyID) {
...
ActiveInstance * ActiveInstanceManager::CreateInstance(int zoneID,
...
void ActiveInstance::RecruitSidekick(CreatureInstance *recruit,
		CreatureInstance *host, SidekickObject &skobj) {
...
CreatureInstance* ActiveInstance::InstantiateSidekick(CreatureInstance *host,
		SidekickObject &skobj, int count) {
...
int ActiveInstance::CreateSidekick(CreatureInstance* host,
...
int ActiveInstance::SidekickRemove(CreatureInstance* host,
		vector<SidekickObject> *sidekickList, int CID) {
	int size = 0;
	list<CreatureInstance>::iterator it;
	for (it = SidekickList.begin(); it != SidekickList.end(); ++it) {
		if (it->CreatureID == CID) {
			...
			LSendToLocalSimulator(GAuxBuf, size, host->CurrentX,
					host->CurrentZ);
			EraseAllCreatureReference(&*it);
...
int ActiveInstance::SidekickRemoveOne(CreatureInstance* host,
		vector<SidekickObject> *sidekickList) {
...
int ActiveInstance::SidekickRemoveAll(CreatureInstance* host,
		vector<SidekickObject> *sidekickList) {
...
int ActiveInstance::SidekickRegister(CreatureInstance* host,
		vector<SidekickObject> *sidekickList) {
	...
	for (size_t i = 0; i < sidekickList->size(); i++) {
		if (InstantiateSidekick(host, sidekickList->at(i), count) != NULL) {
			g_Logs.server->info("Registering sidekick: %v",
					sidekickList->at(i).CDefID);
			count++;
		}
	}
...
void ActiveInstance::SidekickDefend(CreatureInstance* host) {
	...
	for (a = 0; a < SidekickListPtr.size(); a++)
		if (SidekickListPtr[a]->AnchorObject == host) {
			SidekickListPtr[a]->SetServerFlag(ServerFlags::Noncombatant, -1);
			SidekickListPtr[a]->SetServerFlag(ServerFlags::Defender, 1);
			if (host->GetHateProfile() == NULL
					|| host->GetHateProfile()->hateList.size() == 0) {
	...
	for (a = 0; a < SidekickListPtr.size(); a++)
		if (SidekickListPtr[a]->AnchorObject == host) {
			...
			if (size > sizeof(GSendBuf) - 1000) {
				LSendToLocalSimulator(GSendBuf, size, host->CurrentX,
						host->CurrentZ);
				size = 0;
			}
		}
	...
	for (a = 0; a < SidekickListPtr.size(); a++) {
		if (SidekickListPtr[a]->AnchorObject == host) {
			if (SidekickListPtr[a]->NotStatus(StatusEffects::DEAD) == true) {
				...
				int hpercent = (int) (((float) SidekickListPtr[a]->css.health
						/ (float) maxhealth) * 100);
	...
	for (a = 0; a < (int) SidekickListPtr.size(); a++) {
		if (SidekickListPtr[a]->AnchorObject == host) {
			...
			wpos += PutStringUTF(&outBuf[wpos],
					SidekickListPtr[a]->css.display_name);
			size++;
		}
	}
...
int ActiveInstance::DetachSceneryEffect(char *outBuf, int sceneryId,
		int effectType, int tag) {
	int wpos = 0;
...
SceneryEffectList* ActiveInstance::GetSceneryEffectList(int PropID) {
	SceneryEffectMap::iterator it = mSceneryEffects.find(PropID);
	if (it == mSceneryEffects.end()) {
		return &mSceneryEffects.insert(
				SceneryEffectMap::value_type(PropID, SceneryEffectList())).first->second;
	}
	return &it->second;
}
...
void ActiveInstance::UpdateSidekickTargets(CreatureInstance *officer) {
	...
	if (officer->CurrentTarget.targ == NULL)
		return;
	if (officer->_ValidTargetFlag(officer->CurrentTarget.targ,
			TargetStatus::Enemy_Alive) == false)
		return;
...
void ActiveInstance::SendLoyaltyAggro(CreatureInstance *instigator,
		CreatureInstance *target, int loyaltyRadius) {
	...
	for (it = NPCList.begin(); it != NPCList.end(); ++it) {
		...
		g_Logs.server->info("Loyalty aggro: %v (%v range)",
				source->css.display_name, loyaltyRadius);
	}
	//g_Log.AddMessageFormat("Loyalty aggro: %d (%d range)", count, loyaltyRadius);
}

void ActiveInstance::SendLoyaltyLinks(CreatureInstance *instigator,
		CreatureInstance *target, SceneryObject *spawnPoint) {
	if (spawnPoint == NULL)
		return;
	...
	for (size_t i = 0; i < openList.size(); i++) {
		...
		if (so->IsSpawnPoint() == false)
			continue;
		spawnsys.EnumAttachedCreatures(so->ID, (int) so->LocationX,
				(int) so->LocationZ, creatureIDs);
	}
...
void ActiveInstance::SendPlaySound(const char *assetPackage,
...
bool ActiveInstance::QueueCallUsed(int sourceCreatureID, int usedCreatureID,
		int usedCreatureDefID, int time) {
	if (nutScriptPlayer != NULL) {
...
bool ActiveInstance::ScriptCallUse(int sourceCreatureID, int usedCreatureID,
...
void ActiveInstance::ScriptCallUseHalt(int sourceCreatureID,
	...
	if (nutScriptPlayer != NULL) {
		...
		Util::SafeFormat(buffer, sizeof(buffer), "onUseHalt_%d",
				usedCreatureDefID);
		ScriptCall(buffer);
	}
}

void ActiveInstance::ScriptCallUseFinish(int sourceCreatureID,
	...
	if (nutScriptPlayer != NULL) {
		...
		Util::SafeFormat(buffer, sizeof(buffer), "on_use_finish_%d",
				usedCreatureDefID);
		...
		Util::SafeFormat(buffer, sizeof(buffer), "onUseFinish_%d",
				usedCreatureDefID);
		ScriptCall(buffer);
	}
}
...
bool ActiveInstance::RunScript(std::string &errors) {
	if ((scriptPlayer != NULL && scriptPlayer->mActive)
		...
		g_Logs.script->warn(
				"Request to run script for %v when it is already running",
				mZone);
		return false;
	}
...
bool ActiveInstance::KillScript() {
	...
	ClearScriptObjects();
	if (!ok) {
		g_Logs.script->warn("Request to kill inactive squirrel script %v",
				mZone);
	}
	return ok;
}

void ActiveInstance::FetchNearbyCreatures(SimulatorThread *simPtr,
		CreatureInstance *player) {
	int wpos = 0;
#ifdef CREATUREQAV
	for (size_t i = 0; i < NPCListPtr.size(); i++) {
		...
		if ((xlen < g_CreatureListenRange) && (zlen < g_CreatureListenRange)) {
			wpos += PrepExt_GeneralMoveUpdate(&simPtr->SendBuf[wpos],
					NPCListPtr[i]);
...
void ActiveInstance::RunObjectInteraction(SimulatorThread *simPtr, int CID) {
	...
	if (cinst == NULL)
		...
		InteractObject *intObj = g_InteractObjectContainer.GetObjectByID(
				cinst->CreatureDefID, simPtr->pld.CurrentZoneID);
		if (intObj != NULL) {
		...
		ScriptCallUseFinish(simPtr->creatureInst->CreatureID,
				cinst->CreatureDefID, cinst->CreatureID);
	}
}
...
CreatureInstance* ActiveInstance::GetMatchingSidekick(CreatureInstance *host,
		int searchID) {
	for (size_t i = 0; i < SidekickListPtr.size(); i++)
...
void ActiveInstance::SetAllPlayerPVPStatus(int x, int z, int range,
		bool state) {
	for (size_t i = 0; i < PlayerListPtr.size(); i++) {
		if (range == -1
				|| ActiveInstance::GetPointRangeXZ(PlayerListPtr[i], (float) x,
						(float) z, range) < DISTANCE_FAILED) //Half radius of minimap.
						{
			if (state == true)
...
int ActiveInstance::ActivateAbility(CreatureInstance *cInst, short ability,
		int ActionType, ActiveAbilityInfo *abInfo) {
	...
	memcpy(Debug::ActivateAbility_abTargetList, cInst->ab[0].TargetList,
			sizeof(cInst->ab[0].TargetList));
	...
	int res = g_AbilityManager.ActivateAbility(cInst, ability, ActionType,
			abInfo);
	g_AbilityManager.RunImplicitActions();
	return res;
}

ActiveInstance* ActiveInstanceManager::ResolveExistingInstance(
		PlayerInstancePlacementData& pd, ZoneDefInfo* zoneDef) {
	if (zoneDef == NULL)
		return NULL;
	...
	if (pd.in_instanceID != 0) {
		...
		if (ptr != NULL) {
			g_Logs.server->debug("Found instance by instance ID: %v",
					pd.in_instanceID);
			return ptr;
		}
	}
	...
	if (zoneDef->mInstance == true) {
		...
		if (pd.in_partyID != 0) {
			...
			if (ptr != NULL) {
				g_Logs.server->debug("Found instance by party ID: %v",
						pd.in_partyID);
				return ptr;
			}
		...
		if (ptr != NULL) {
			g_Logs.server->debug("Found instance by owner: %v",
					pd.in_creatureDefID);
			return ptr;
		}
	} else {
...
int ActiveInstanceManager::AddSimulator_Ex(PlayerInstancePlacementData &pd) {
	...
	if (zoneDef == NULL) {
		g_Logs.server->debug("AddSimulator() ZoneDef does not exist for ID: %v",
				pd.in_zoneID);
		return -1;
	}
	...
	if (ptr == NULL) {
		g_Logs.server->debug("Creating new instance for zone: %v",
				pd.in_zoneID);
	...
	if (ptr == NULL) {
		g_Logs.server->error(
				"AddSimulator() Unable to create instance for zone: %v",
				pd.in_zoneID);
		return -1;
	}
...
void PlayerInstancePlacementData::SetInstanceScaler(const std::string &name) {
	...
	if (profile == NULL) {
		if (in_partyID == 0
				|| (in_partyID != 0
						&& mPartyLeaderDefID == this->in_creatureDefID)) {
...
const InstanceScaleProfile* PlayerInstancePlacementData::GetPartyLeaderInstanceScaler(
		void) {
	mPartyLeaderDefID = 0;
...
OnUsedCallback::OnUsedCallback(ActiveInstance *instance, int sourceCreatureID,
		int usedCreatureDefID, int usedCreatureID) {
	mInstance = instance;
