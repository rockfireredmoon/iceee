...
std::string AbstractAppearanceModifier::DoModify(std::string source, bool eq) {
	...
	if (pos == string::npos) {
		g_Logs.server->error("Could not parse exist appearance. %v",
				currentAppearance.c_str());
		return source;
	}
	string prefix = currentAppearance.substr(0, pos + 1);
	currentAppearance = "this.a <- "
			+ currentAppearance.substr(pos + 1, currentAppearance.size()) + ";";
	...
	if (Sqrat::Error::Occurred(vm)) {
		g_Logs.server->error("Failed to compile appearance. %v",
...
void CreatureAttributeModifier::ProcessTable(Sqrat::Table *table) {
	g_Logs.server->debug("Setting attr '%v' to '%v'", mAttribute.c_str(),
			mValue.c_str());
...
void CreatureDefinition::CopyFrom(CreatureDefinition& source) {
	...
	DefaultEffects.assign(source.DefaultEffects.begin(),
			source.DefaultEffects.end());
...
void CreatureInstance::UnloadResources(void) {
	...
	if (actInst == NULL)
		g_Logs.server->error(
				"UnloadResources() called for creature without an instance.");
	...
	if (serverFlags & ServerFlags::IsPlayer) {
		if (activeLootID != 0) {
			actInst->tradesys.CancelTransaction(CreatureID, activeLootID,
					GSendBuf);
			activeLootID = 0;
		}
	}
	...
	if (serverFlags & ServerFlags::IsNPC) {
		if (spawnGen != NULL) {
			...
			if (spawnGen->spawnPackage != NULL) {
				if (spawnGen->spawnPackage->isScriptCall == true
						&& wasKilled == true)
					actInst->ScriptCallPackageKill(
							spawnGen->spawnPackage->packageName);
				if (spawnGen->spawnPackage->isSequential == true)
					notifyKill = true;
			}

			if (actInst->spawnsys.NotifyKill(spawnGen, CreatureID) == true
					|| notifyKill == true)
				if (wasKilled == true)
...
void CreatureInstance::CopyFrom(CreatureInstance *source) {
	...
	activeStatMod.assign(source->activeStatMod.begin(),
			source->activeStatMod.end());
	activeStatusEffect.assign(source->activeStatusEffect.begin(),
			source->activeStatusEffect.end());
	implicitActions.assign(source->implicitActions.begin(),
			source->implicitActions.end());
...
bool CreatureInstance::StartAI(std::string &errors) {
	...
	if ((cdef != NULL))	//&& !(serverFlags & ServerFlags::Noncombatant))
	{
		...
		if (def == NULL) {
			...
			if (aiScript == NULL)
				g_Logs.script->warn(
						"Could not find script [%v] for instantiated creature [%v]",
						p.mScriptName.c_str(), cdef->css.display_name);
			else
			...
			if (aiNut == NULL) {
				g_Logs.script->warn(
						"While the script %v for instantiated creature [%v] was found, it did not start.",
						p.mScriptName.c_str(), cdef->css.display_name);
			}
		}
...
void CreatureInstance::Instantiate(void) {
	...
	if ((serverFlags & ServerFlags::KillableProp) == 0) {
		if ((strlen(css.ai_package) == 0)
				|| (strcmp(css.ai_package, "nothing") == 0))
	...
	if (css.channeling_break_chance == 0)
		css.channeling_break_chance =
				ABGlobals::DEFAULT_CHANNELING_BREAK_CHANCE;
...
int CreatureInstance::GetMitigatedDamage(int damageAmount, int armorRating,
		int reductionMod) {
	...
	if (reductionMod != 0) {
		float multiplier = (float) reductionMod
				/ (float) ABGlobals::INTEGRAL_FRACTION_TOTAL;
...
bool CreatureInstance::IsValidForPVP(void) {
	return (serverFlags & ServerFlags::IsPlayer)
			&& actInst->mMode != PVP::GameMode::PVE_ONLY
			&& _HasStatusList(StatusEffects::PVPABLE) != -1;
}
...
int CreatureInstance::GetAdditiveWeaponSpecialization(int amount,
		int itemEquipSlot) {
	if (!(serverFlags & ServerFlags::IsPlayer))
		return 0;
	...
	case WeaponType::SMALL:
		add = Util::GetAdditiveFromIntegralPercent1000(amount,
				css.weapon_damage_small);
		break;
	case WeaponType::ONE_HAND:
		add = Util::GetAdditiveFromIntegralPercent1000(amount,
				css.weapon_damage_1h);
		break;
	case WeaponType::TWO_HAND:
		add = Util::GetAdditiveFromIntegralPercent1000(amount,
				css.weapon_damage_2h);
		break;
	case WeaponType::POLE:
		add = Util::GetAdditiveFromIntegralPercent1000(amount,
				css.weapon_damage_pole);
		break;
	case WeaponType::WAND:
		add = Util::GetAdditiveFromIntegralPercent1000(amount,
				css.weapon_damage_wand);
		break;
	case WeaponType::BOW:
		add = Util::GetAdditiveFromIntegralPercent1000(amount,
				css.weapon_damage_box);
		break;
	case WeaponType::THROWN:
		add = Util::GetAdditiveFromIntegralPercent1000(amount,
				css.weapon_damage_thrown);
		break;
...
int CreatureInstance::GetAdjustedDamage(CreatureInstance *attacker,
		int damageType, int amount) {
	if (amount == 0)
		return 0;
	...
	case DamageType::MELEE:
		...
		amount = GetMitigatedDamage(amount, css.damage_resist_melee,
				css.dr_mod_melee);
		break;
	case DamageType::FIRE:
		...
		amount = GetMitigatedDamage(amount, css.damage_resist_fire,
				css.dr_mod_fire);
		break;
	case DamageType::FROST:
		...
		amount -= Combat::GetPsycheResistReduction(amount, css.psyche);
		amount = GetMitigatedDamage(amount, css.damage_resist_frost,
				css.dr_mod_frost);
		break;
	case DamageType::MYSTIC:
		...
		amount = GetMitigatedDamage(amount, css.damage_resist_mystic,
				css.dr_mod_mystic);
		break;
	case DamageType::DEATH:
		...
		amount -= Combat::GetSpiritResistReduction(amount, css.spirit);
		amount = GetMitigatedDamage(amount, css.damage_resist_death,
				css.dr_mod_death);
		break;
	}
	return amount;
...
int CreatureInstance::ApplyPostDamageModifiers(CreatureInstance *attacker,
		int amount, int &absorbedDamage) {
	if (amount == 0)
		return 0;
	...
	if (css.bonus_health > 0) {
		...
		//next processed.
		SubtractAbilityBuffStat(STAT::BONUS_HEALTH, -1,
				static_cast<float>(subt));
		//css.bonus_health -= subt;
		absorbedDamage += subt;
		pendingOperations.UpdateList_Add(CREATURE_UPDATE_STAT, this,
				STAT::BONUS_HEALTH);
	}
	return amount;
}
...
//	{
	...
	if (hprof != NULL) {
		...
		//be more reasonable.
		float maxHealth = (float) ((css.constitution * HealthConModifier)
				+ css.base_health);
		if (maxHealth < 1.0F)
			maxHealth = 1.0F;
		...
		if (attacker->css.hate_gain_rate != 0)
			hateAmount += Util::GetAdditiveFromIntegralPercent100(
					(int) hateAmount, attacker->css.hate_gain_rate);

		if (attacker->AnchorObject != NULL) {
			// Decide how much to hate the sidekick itself and the officer

			if (attacker->sidekickData.summonType
					== SidekickObject::HATE_SIDEKICK
					|| attacker->sidekickData.summonType
							== SidekickObject::HATE_BOTH)
			...
			if (attacker->sidekickData.summonType
					...
					|| attacker->sidekickData.summonType
							== SidekickObject::HATE_BOTH)
				hprof->Add(attacker->CreatureID, attacker->CreatureDefID, level,
						amount, (int) hateAmount);

			if (attacker->sidekickData.summonType
					== SidekickObject::HATE_OFFICER_MORE) {
				hprof->Add(CID, CDefID, level, amount,
						(int) (hateAmount / 2.0));
				hprof->Add(attacker->CreatureID, attacker->CreatureDefID, level,
						amount, (int) hateAmount);
			} else if (attacker->sidekickData.summonType
					== SidekickObject::HATE_OFFICER_MORE) {
				hprof->Add(CID, CDefID, level, amount, (int) hateAmount);
				hprof->Add(attacker->CreatureID, attacker->CreatureDefID, level,
						amount, (int) (hateAmount / 2.0));
			}
		} else {
			hprof->Add(CID, CDefID, level, amount, (int) hateAmount);
		}
...
//	}
	...
	if ((serverFlags & ServerFlags::IsPlayer)
			&& (serverFlags & ServerFlags::Defended)) {
...
void CreatureInstance::CheckStatusInterruptOnHit(void) {
	if (HasStatus(StatusEffects::DAZE)) {
		...
		if (index >= 0)
			activeStatusEffect[index].expireTime =
					activeStatusEffect[index].startTime
							+ Global::DazeGuaranteeTime;
...
void CreatureInstance::CheckMovement(float movementStep) {
	...
	if (HasStatus(StatusEffects::WALK_IN_SHADOWS)
			|| HasStatus(StatusEffects::INVISIBLE)) {
...
void CreatureInstance::CastSetback(void) {
	...
	int size = PrepExt_AbilityActivate(GSendBuf, this, &ab[0],
			AbilityStatus::SETBACK);
...
bool CreatureInstance::RegisterHostility(CreatureInstance *attacker,
		int hostility) {
	if (attacker == NULL) {
	...
	if (aiScript != NULL || aiNut != NULL) {
		if (((serverFlags & ServerFlags::CalledBack) == false)
				&& ((serverFlags & ServerFlags::LeashRecall) == false)) {
...
bool CreatureInstance::NotSilenced(void) {
	...
	if (statusEffects[StatusEffectBitData[StatusEffects::SILENCE].arrayIndex]
			& StatusEffectBitData[StatusEffects::SILENCE].bit)
		return false;
...
bool CreatureInstance::isTargetInCone(CreatureInstance * target,
		double amount) {
...
bool CreatureInstance::CheckBuffLimits(int level, int buffCategory,
		bool addBuff) {
...
bool CreatureInstance::InRange(float dist, bool useGroundLocation) {
	...
	//Hack for GTAE abilities
	if (useGroundLocation == true) {
		float locDist = (float) actInst->GetPointRangeXZ(this, ab[0].x, ab[0].z,
				(int) dist);
		return (locDist <= dist);
	}
	...
	float actualDist = (float) ActiveInstance::GetPlaneRange(this, target,
			(int) acceptDist);
...
bool CreatureInstance::InRange_Target(float dist) {
	...
	//Don't include Y axis since the server doesn't know about elevation differences.
	float actualDist = (float) actInst->GetPlaneRange(this, CurrentTarget.targ,
			(int) acceptDist);
	if (actualDist <= acceptDist)
		return true;
...
bool CreatureInstance::NotStatus(int statusID) {
	...
	if (statusEffects[StatusEffectBitData[statusID].arrayIndex]
			& StatusEffectBitData[statusID].bit)
		return false;
...
void CreatureInstance::_SetStatusFlag(int statusID) {
	statusEffects[StatusEffectBitData[statusID].arrayIndex] |=
			StatusEffectBitData[statusID].bit;
	/*
	 if(send == true)
	 SendFlags();
...
int CreatureInstance::Add(unsigned char tier, unsigned char buffCategory,
		int abID, int abgID, int statID, float calcAmount, float descAmount,
		float durationSec) {
	...
	ActiveBuff *activeBuff = buffManager.UpdateBuff(tier, buffCategory, abID,
			abgID, durationSec, initialisingAbilities);

	AddBuff(BuffSource::ABILITY, buffCategory, tier, abID, abgID, statID,
			calcAmount, descAmount, activeBuff->durationS);
...
void CreatureInstance::AddBuff(int buffSource, int buffCategory, int abTier,
		int abID, int abGroupID, int statID, float calcAmount, float descAmount,
		float durationSec) {
	...
	pendingOperations.UpdateList_Add(CREATURE_UPDATE_MOD | CREATURE_UPDATE_STAT,
			this, statID);
}
...
ActiveBuff * CreatureInstance::AddMod(unsigned char tier, int buffCategory,
		short abID, short abgID, double durationSec) {
	ActiveStatMod newMod;
	...
	ActiveBuff * buff = buffManager.UpdateBuff(tier, buffCategory, abID, abgID,
			durationSec, initialisingAbilities);
...
void CreatureInstance::SubtractAbilityBuffStat(int statID, int abgID,
		float amount) {
	...
	if (r >= 0) {
		...
		if (statID == STAT::BONUS_HEALTH && (newAmount < 1.0F)) {
			...
			pendingOperations.UpdateList_Add(
					CREATURE_UPDATE_MOD | CREATURE_UPDATE_STAT, this, statID);
		}
	}
...
void CreatureInstance::RemoveAllBuffsExceptGroupID(bool send,
		int abilityGroupID) {
	size_t pos = 0;
	while (pos < activeStatMod.size()) {
...
void CreatureInstance::RemoveAbilityBuffTypeExcept(int buffCategory,
		int abilityID, int abilityGroupID) {
	...
	while (pos < activeStatMod.size()) {
		del = false;
		if (activeStatMod[pos].sourceType == BuffSource::ABILITY
				&& activeStatMod[pos].buffCategory == buffCategory) {
	...
	if (abilityCanceled == true) {
		int wpos = PrepExt_AbilityEvent(GSendBuf, CreatureID, abilityID,
				AbilityStatus::INTERRUPTED);
		actInst->LSendToLocalSimulator(GSendBuf, wpos, CurrentX, CurrentZ);
	}
}
...
bool CreatureInstance::RemoveAbilityBuffWithStat(int statID, float sign) {
	...
	while (pos < activeStatMod.size()) {
		del = false;
		if (activeStatMod[pos].sourceType == BuffSource::ABILITY
				&& activeStatMod[pos].modStatID == statID) {
			if ((sign < 0.0F && activeStatMod[pos].amount < 0.0F)
					|| (sign > 0.0F && activeStatMod[pos].amount > 0.0F)) {
	...
	if (foundAbilityID != 0) {
		...
		for (size_t i = 0; i < activeStatMod.size(); i++) {
			if (activeStatMod[i].sourceType == BuffSource::ABILITY
					&& activeStatMod[i].abilityID == foundAbilityID)
				count++;
		}

		//If there are none, notify to remove the buff.
		if (count == 0) {
			int wpos = PrepExt_AbilityEvent(GSendBuf, CreatureID,
					foundAbilityID, AbilityStatus::INTERRUPTED);
			actInst->LSendToLocalSimulator(GSendBuf, wpos, CurrentX, CurrentZ);
			SendUpdatedBuffs();
			return true;
		}
...
int CreatureInstance::CalcRestrictedHealth(int health, bool addmod) {
	...
	if (css.max_health_pc != 100) {
		int maxHealth = (int) (((float) GetMaxHealth(addmod) / 100.0)
				* css.max_health_pc);
		if (health > maxHealth)
			health = maxHealth;
	}
	if (css.min_health_pc != 0) {
		int minHealth = (int) (((float) GetMaxHealth(addmod) / 100.0)
				* css.min_health_pc);
		if (health < minHealth)
			health = minHealth;
	}
...
void CreatureInstance::LimitValueOverflows(void) {
	...
	int healthValueCap = (
			(g_Config.UseIntegerHealth == true) ? MAX_INT : MAX_SHORT);
	...
	if (max > healthValueCap) {
		...
		int rarity = Util::ClipInt(css.rarity, 0, MAX_RARITY_INDEX);
		float healthPerCon = HealthConModifier
				* RarityTypeHealthModifier[rarity];
		float trimCon = (overflowHealth / healthPerCon) + 1;
		css.constitution -= (int) trimCon;
		int newHP = CalcRestrictedHealth(-1, true);
		css.health = newHP;
		g_Logs.server->warn(
				"Constitution overflow! OldHP:%v, ConReduct:%v, NewHP:%v", max,
				(int) trimCon, newHP);
	}

	css.damage_resist_melee = Util::ClipInt(css.damage_resist_melee, 0,
			MAX_SHORT);
	css.damage_resist_fire = Util::ClipInt(css.damage_resist_fire, 0,
			MAX_SHORT);
	css.damage_resist_frost = Util::ClipInt(css.damage_resist_frost, 0,
			MAX_SHORT);
	css.damage_resist_mystic = Util::ClipInt(css.damage_resist_mystic, 0,
			MAX_SHORT);
	css.damage_resist_death = Util::ClipInt(css.damage_resist_death, 0,
			MAX_SHORT);
...
float CreatureInstance::GetHealthRatio(void) {
	return static_cast<float>(css.health)
			/ static_cast<float>(GetMaxHealth(true));
}

void CreatureInstance::RunHealTick(void) {
	double regen = (css.spirit * RegenSpiritMod)
			+ (css.constitution * RegenConMod) + (css.level * RegenLevelMod);
...
bool CreatureInstance::CanUseAnyWeapon(void) {
	if ((serverFlags & ServerFlags::IsNPC)
			|| (serverFlags & ServerFlags::IsSidekick))
		return true;
	return false;
}

bool CreatureInstance::hasMainHandWeapon(void) {
	if (CanUseAnyWeapon() == true) {
		return true;
	}
...
bool CreatureInstance::has2HorPoleWeapon(void) {
	...
	return ((EquippedWeapons[ItemEquipSlot::WEAPON_MAIN_HAND]
			== WeaponType::TWO_HAND)
			|| (EquippedWeapons[ItemEquipSlot::WEAPON_MAIN_HAND]
					== WeaponType::POLE));
}
...
void CreatureInstance::AddHate(CreatureInstance *attacker, int amount) {
	...
	if (hprof == NULL)
		return;
	hprof->Add(attacker->CreatureID, attacker->CreatureDefID,
			attacker->css.level, 0, amount);
...
void CreatureInstance::Amp(unsigned char tier, unsigned char buffType, int abID,
		int abgID, int statID, float percent, int time) {
	...
	if ((StatList[statIndex].etype == StatType::SHORT)
			|| (StatList[statIndex].etype == StatType::INTEGER))
		amount = Util::Round(amount);

	Add(tier, buffType, abID, abgID, statID, amount, percent, time);
}

void CreatureInstance::Set(unsigned char tier, unsigned char buffType, int abID,
		int abgID, int statID, float amount, int time) {
	//Get integer value, run the percent, and forward that amount and everything
	//else into the Add() function, which handles the actual buff management.
...
void CreatureInstance::GoSanctuary(void) {
	...
	const char *regionName = MapLocation.GetInternalMapName(actInst->mZone,
			CurrentX, CurrentZ);
	WorldCoord* sanct = g_ZoneMarkerDataManager.GetNearestRegionSanctuaryInZone(
			regionName, actInst->mZone, CurrentX, CurrentZ);
	if (sanct != NULL) {
		...
		if (serverFlags & ServerFlags::IsPlayer) {
			int wpos = PrepExt_SendInfoMessage(GSendBuf,
					"No sanctuary in range", INFOMSG_ERROR);
...
void CreatureInstance::OnUnstick(void) {
	...
	g_Logs.event->info("[UNSTICK] %v (%v,%v,%v,%v)", css.display_name,
			ZoneString, CurrentX, CurrentY, CurrentZ);
...
void CreatureInstance::RemoveAltBuff(void) {
	...
	const int stats[3] = { STAT::MELEE_ATTACK_SPEED, STAT::MAGIC_ATTACK_SPEED,
			STAT::MOD_MOVEMENT };
	for (int i = 0; i < 3; i++) {
...
void CreatureInstance::RemoveStatBuff(void) {
	...
	const int stats[6] = { STAT::STRENGTH, STAT::DEXTERITY, STAT::CONSTITUTION,
...
void CreatureInstance::RemoveHealthBuff(void) {
	...
	const int stats[3] = { STAT::BASE_HEALTH, STAT::BONUS_HEALTH,
			STAT::CONSTITUTION, };
	for (int i = 0; i < 3; i++) {
		if (RemoveAbilityBuffWithStat(stats[i], 1.0F) == true)
			return;
...
void CreatureInstance::RemoveEtcBuff(void) {
	...
	const int stats[10] = { STAT::BASE_DAMAGE_FIRE, STAT::BASE_DAMAGE_FROST,
			STAT::BASE_DAMAGE_MYSTIC, STAT::BASE_DAMAGE_DEATH,
			STAT::DAMAGE_RESIST_MELEE, STAT::DAMAGE_RESIST_FIRE,
			STAT::DAMAGE_RESIST_FROST, STAT::DAMAGE_RESIST_MYSTIC,
			STAT::DAMAGE_RESIST_DEATH, STAT::BASE_HEALTH
...
void CreatureInstance::RemoveAltDebuff(void) {
	...
	const int stats[3] = { STAT::MELEE_ATTACK_SPEED, STAT::MAGIC_ATTACK_SPEED,
			STAT::MOD_MOVEMENT };
	const int effects[3] = { StatusEffects::FEAR, StatusEffects::CHARM,
			StatusEffects::ROOT };
	for (int i = 0; i < 3; i++) {
		if (RemoveAbilityBuffWithStat(stats[i], -1.0F) == true)
			return;
...
void CreatureInstance::PortalRequest(CreatureInstance *caster,
		const char *internalname, const char *externalname) {
...
void CreatureInstance::BindTranslocate(void) {
	...
	WorldCoord* sanct = g_ZoneMarkerDataManager.GetSanctuaryInRange(
			actInst->mZone, CurrentX, CurrentZ, 250);
	if (sanct == NULL) {
		int wpos = PrepExt_SendInfoMessage(GSendBuf,
				"You must be near a sanctuary to bind.", INFOMSG_ERROR);
		simulatorPtr->AttemptSend(GSendBuf, wpos);
	} else {
		charPtr->bindReturnPoint[0] = (int) sanct->x;
		charPtr->bindReturnPoint[1] = (int) sanct->y
				+ SANCTUARY_ELEVATION_ADDITIVE;
...
void CreatureInstance::Resurrect(float healthratio, float luckratio,
		int abilityID) {
	...
	if (actInst->mZoneDefPtr->mArena == true && rezScreen == true) {
		...
		AddBuff(BuffSource::INSTANCE, 0, 0, 0, 0, STAT::MOD_MOVEMENT, 40, 40,
				6);
	}

	css.hide_nameboard = 0;
	const short stats[4] = { STAT::HEALTH, STAT::HIDE_NAMEBOARD,
			STAT::BASE_LUCK, STAT::HEROISM };
...
void CreatureInstance::ProcessPVPGoal() {
	if (actInst != NULL && actInst->pvpGame != NULL) {
		...
		if (team != NULL) {
			...
			if (member != NULL) {
				member->mPVPGoals++;
				actInst->LSendToLocalSimulator(GSendBuf,
						PrepExt_PVPStatUpdate(GSendBuf, actInst->pvpGame,
								member), CurrentX, CurrentZ);
			}
		}
	}
}
...
void CreatureInstance::ProcessDeath(void) {
		...
		if (team != NULL) {
			...
			for (size_t i = 0; i < attackerList.size(); i++) {
				...
				if (attackerTeam != NULL) {
					...
					PartyMember * attackerMember = team->GetMemberByID(
							attacker->CreatureID);
					if (attackerMember != NULL) {
						attackerMember->mPVPKills++;
						actInst->LSendToLocalSimulator(GSendBuf,
								PrepExt_PVPStatUpdate(GSendBuf,
										actInst->pvpGame, attackerMember),
								CurrentX, CurrentZ);
					}
				}
			}
	...
	if (actInst->mZoneDefPtr->mGrove == false) {
		...
		if (serverFlags & ServerFlags::IsPlayer) {
			...
			AccountData *victimsAccount = g_AccountManager.GetActiveAccountByID(
					charPtr->AccountID);
			if (victimsAccount == NULL) {
				g_Logs.server->warn(
						"Cannot find account %v for character %v to update kill/death stats.",
						charPtr->AccountID);
			}
			...
			if (charPtr->Mode == PVP::GameMode::PVP
				...
				for (size_t i = 0; i < attackerList.size(); i++) {
					...
					if (attacker->charPtr != NULL
							&& attacker->charPtr->AccountID > 0) {
						AccountData *attackersAccount =
								g_AccountManager.GetActiveAccountByID(
										attacker->charPtr->AccountID);
				...
				if (!npcInvolved && pvpAttackers.size() > 0) {
					...
					if (items > 0) {
						...
						for (int i = 0; i < items; i++) {
							...
							InventorySlot *slot = origInv->PickRandomItem(
									INV_CONTAINER);
							if (slot == NULL)
								break;
							...
							for (int i = 0;
									i < toLoot && loot.itemList.size() < 16;
									i++)
								loot.AddItem(slot->IID);

							// Remove from player
							int len = origInv->RemoveItemsAndUpdate(
									INV_CONTAINER, slot->IID, toLoot, buffer);
							if (len > 0)
						...
						if (loot.itemList.size() == 0)
							g_Logs.server->warn(
									"PVP resulted in no loot because losing player had no unbound items");
						else {
							 ...
							 */
							CreatureInstance* lootInst = actInst->SpawnGeneric(
									7861, CurrentX, CurrentY, CurrentZ, 0, 0);
							lootInst->deathTime = g_ServerTime;
							lootInst->PrepareDeath();
							loot.CreatureID = lootInst->CreatureID;

							lootInst->activeLootID =
									actInst->lootsys.AttachLootToCreature(loot,
											lootInst->CreatureID);
		...
		if (creditDrops > 0) {
			...
			if (credits < 0) {
				g_Logs.server->debug(
						"Party earned no credits as level difference too great or nobody was left alive");
			} else {
				if (credits == 1)
					Util::SafeFormat(buf, sizeof(buf),
							"Your party earned 1 credit each");
				else if (credits > 1)
					Util::SafeFormat(buf, sizeof(buf),
							"Your party earned %d credits each", credits);

				g_Logs.server->debug(
						"This mob drops credits. The team earned %v (max player level %v, difference %v, alive %v)",
						credits, maxPlayerLevel, levelDiff, alivePlayers);
			}
			...
			if (attacker->HasStatus(StatusEffects::DEAD) == false) {
				...
				if (!(serverFlags & ServerFlags::IsPlayer)) {
					...
					if (attacker->css.experience_gain_rate > 0) {
						exp += Util::GetAdditiveFromIntegralPercent100(exp,
								attacker->css.experience_gain_rate);
					}
					...
					if (credits > 0) {
						...
						int wpos = PrepExt_SendInfoMessage(GSendBuf, buf,
								INFOMSG_INFO);
...
void CreatureInstance::AddCreaturePointer(CREATURE_SEARCH& output,
		CreatureInstance* ptr, int attacked) {
	if (ptr == NULL)
		return;
		...
		if (creatureList[i].ptr->PartyID != 0) {
			...
			if (party != NULL) {
				...
				for (size_t pm = 0; pm < party->mMemberList.size(); pm++) {
					...
					if (actInst->GetPlaneRange(this, lookup,
							PARTY_SHARE_DISTANCE) >= PARTY_SHARE_DISTANCE)
						continue;
...
void CreatureInstance::ResolveAttackers(CREATURE_SEARCH& results) {
	...
	for (size_t i = 0; i < hate->hateList.size(); i++) {
		...
		if (lookup == NULL)
			continue;
		int attacked = (hate->hateList[i].damage
				> (lookup->GetMaxHealth(false) / 10));
		...
		for (size_t pm = 0; pm < party->mMemberList.size(); pm++) {
			...
			if (actInst->GetPlaneRange(this, lookup, PARTY_SHARE_DISTANCE)
					>= PARTY_SHARE_DISTANCE)
				continue;
...
void CreatureInstance::_UpdateHealthMod(void) {
	...
	css.health_mod = (int) (((double) css.heroism / 10000.0F)
			* (double) maxhealth);
...
void CreatureInstance::UpdateAggroPlayers(short state) {
	...
	if (HasStatus(StatusEffects::UNATTACKABLE)
			|| HasStatus(StatusEffects::INVINCIBLE))
		return;
...
void CreatureInstance::AddWDesc(int statID, int unknown1, int unknown2,
		const char *name) {
}
...
void CreatureInstance::Nullify(unsigned char tier, unsigned char buffType,
		int abID, int abgID, int statID, int duration) {
...
void CreatureInstance::AmpCore(unsigned char tier, unsigned char buffType,
		int abID, int abgID, float ratio, int duration) {
	...
	Add(tier, buffType, abID, abgID, STAT::CONSTITUTION, amount, ratio,
			duration);
...
bool CreatureInstance::Translocate(bool test) {
	...
	if (charPtr->bindReturnPoint[3] == 0) {
		int wpos = PrepExt_SendInfoMessage(GSendBuf,
				"You have not bound to a sanctuary.", INFOMSG_ERROR);
		simulatorPtr->AttemptSend(GSendBuf, wpos);
		return false;
	}
	if (test == true)
...
void CreatureInstance::CancelPending(void) {
	...
	if (serverFlags & ServerFlags::IsPlayer) {
		...
		case ABILITYID_QUEST_GATHER_OBJECT:
			...
			QuestScript::QuestNutPlayer *nut =
					actInst->GetSimulatorQuestNutScript(simulatorPtr);
			if (nut != NULL) {
				char buffer[64];
				Util::SafeFormat(buffer, sizeof(buffer), "on_use_cancel_%d",
						CreatureDefID);
	...
	if (registeredAbility == false) {
		int size = 0;
		if (serverFlags & ServerFlags::IsPlayer)
			size = PrepExt_AbilityActivateEmpty(GSendBuf, this, &ab[0],
					AbilityStatus::ABILITY_FINISHED);
		size += PrepExt_AbilityActivateEmpty(&GSendBuf[size], this, &ab[0],
				AbilityStatus::INTERRUPTED);
		actInst->LSendToLocalSimulator(GSendBuf, size, CurrentX, CurrentZ);
	}
...
//			}
	...
	if (registeredAbility == true) {
		int size = 0;
		if (serverFlags & ServerFlags::IsPlayer)
			size += PrepExt_AbilityActivateEmpty(&GSendBuf[size], this, ability,
					AbilityStatus::ABILITY_FINISHED);
		size += PrepExt_AbilityActivateEmpty(&GSendBuf[size], this, ability,
				AbilityStatus::INTERRUPTED);
		actInst->LSendToLocalSimulator(GSendBuf, size, CurrentX, CurrentZ);
...
void CreatureInstance::RegisterChannel(int abilityID, int duration,
		int interval, bool bOverride, int interruptResistMod) {
	if (HasStatus(StatusEffects::DAZE))
		return;
	...
	if (duration < interval)
		g_Logs.server->warn(
				"RegisterChannel() possible iteration problem (ability: %v, duration: %v, interval: %v",
				abilityID, duration, interval);
	...
	if (bOverride == true) {
		...
		int size = 0;
		if (serverFlags & ServerFlags::IsPlayer)
			size = PrepExt_AbilityActivateEmpty(GSendBuf, this, &ab[abSlot],
					AbilityStatus::REQUEST_ACCEPTED);
...
//		size += PrepExt_AbilityActivate(&GSendBuf[size], this, &ab[abSlot], AbilityStatus::ACTIVATE, ground);  //was ACTIVATE
		...
		size += PrepExt_AbilityActivate(&GSendBuf[size], this, &ab[abSlot],
				AbilityStatus::CHANNELING, ground);  //was ACTIVATE
		//actInst->LSendToAllSimulator(GSendBuf, size, -1);
		actInst->LSendToLocalSimulator(GSendBuf, size, CurrentX, CurrentZ);
		if (ab[abSlot].x != 0.0F) {
			ab[abSlot].x = 0.0F;
...
void CreatureInstance::RegisterCast(int abilityID, int warmupTime) {
	...
	if (css.magic_attack_speed != 0)
		time -=
				(int) ((float) time * ((float) css.magic_attack_speed * 0.001F));
	...
	if (serverFlags & ServerFlags::IsPlayer)
		size += PrepExt_AbilityActivateEmpty(&GSendBuf[size], this, &ab[0],
				AbilityStatus::REQUEST_ACCEPTED);
	size += PrepExt_AbilityActivate(&GSendBuf[size], this, &ab[0],
			AbilityStatus::WARMUP);
	actInst->LSendToLocalSimulator(GSendBuf, size, CurrentX, CurrentZ);
}
...
bool CreatureInstance::CanPVPTarget(CreatureInstance *target) {
	...
	if (selfPVP == true && targPVP == true) {
		...
		if (actInst->mZoneDefPtr->mMode == PVP::GameMode::SPECIAL_EVENT
				&& css.pvp_team != target->css.pvp_team)
			return true;

		//If we get here, we are not in an area, and not in a team, check the zone itself to see
		//if PVP is allowed
		if (actInst != NULL && actInst->mZoneDefPtr != NULL
				&& (actInst->mZoneDefPtr->mMode == PVP::GameMode::PVP
						|| actInst->mZoneDefPtr->mMode
...
bool CreatureInstance::_ValidTargetFlag(CreatureInstance *target,
		int abilityRestrict) {
	if (target->serverFlags & ServerFlags::LeashRecall)
		return false;
	if ((target->serverFlags & ServerFlags::Noncombatant)
			&& (abilityRestrict & TargetStatus::TSB_ENEMY))
		return false;

	if (abilityRestrict & TargetStatus::TSB_ENEMY) {
		if ((serverFlags & ServerFlags::IsPlayer)
				&& (target->serverFlags & ServerFlags::IsPlayer))
...
int CreatureInstance::_AddTargetsInCone(double halfAngle, int targetType,
		int distance, int abilityRestrict) {
	int a;
	int count = 0;
#ifdef CREATUREQAV
	for (a = 0; a < (int) actInst->NPCListPtr.size(); a++) {
		if (_ValidTargetFlag(actInst->NPCListPtr[a], abilityRestrict) == true) {
			if (actInst->GetPlaneRange(this, actInst->NPCListPtr[a], distance)
					<= distance) {
	...
	for (a = 0; a < (int) actInst->PlayerListPtr.size(); a++) {
		if (_ValidTargetFlag(actInst->PlayerListPtr[a], abilityRestrict)
				== true) {
			if (actInst->GetPlaneRange(this, actInst->PlayerListPtr[a],
					distance) <= distance) {
				if (isTargetInCone(actInst->PlayerListPtr[a], halfAngle)
						== true) {
					if (count >= MAXTARGET)
						return count;
	...
	for (a = 0; a < (int) actInst->SidekickListPtr.size(); a++) {
		if (_ValidTargetFlag(actInst->SidekickListPtr[a], abilityRestrict)
				== true) {
			if (actInst->GetPlaneRange(this, actInst->SidekickListPtr[a],
					distance) <= distance) {
				if (isTargetInCone(actInst->SidekickListPtr[a], halfAngle)
						== true) {
					if (count >= MAXTARGET)
						return count;
...
//Selected target.
void CreatureInstance::AddTargetsST(CREATURE_PTR_SEARCH &results,
		int abilityRestrict) {
...
void CreatureInstance::AddTargetsConeAngle(CREATURE_PTR_SEARCH &results,
		int abilityRestrict, int distance, double halfAngle) {
	size_t i;
	...
	for (i = 0; i < actInst->PlayerListPtr.size(); i++)
		if (_ValidTargetFlag(actInst->PlayerListPtr[i], abilityRestrict)
				== true)
			...
			if (IsObjectInRange(actInst->PlayerListPtr[i], distance) == true)
				if (isTargetInCone(actInst->PlayerListPtr[i], halfAngle)
						== true)
					results.push_back(actInst->PlayerListPtr[i]);

	for (i = 0; i < actInst->SidekickListPtr.size(); i++)
		if (_ValidTargetFlag(actInst->SidekickListPtr[i], abilityRestrict)
				== true)
//			if(actInst->GetPlaneRange(this, actInst->SidekickListPtr[i], distance) <= distance)
			if (IsObjectInRange(actInst->SidekickListPtr[i], distance) == true)
				if (isTargetInCone(actInst->SidekickListPtr[i], halfAngle)
						== true)
					results.push_back(actInst->SidekickListPtr[i]);
}

//Add targets in the radius around the selected ground location.
void CreatureInstance::AddTargetsGTAE(CREATURE_PTR_SEARCH &results,
		int abilityRestrict, int distance) {
	float x = ab[0].x;
	float z = ab[0].z;
	size_t i;
	for (i = 0; i < actInst->NPCListPtr.size(); i++)
...
//			if(actInst->GetPointRangeXZ(actInst->PlayerListPtr[i], ab[0].x, ab[0].z, distance) <= distance)
			if (actInst->PlayerListPtr[i]->IsSelfNearPoint(x, z, distance)
					== true)
				results.push_back(actInst->PlayerListPtr[i]);
	...
	for (i = 0; i < actInst->SidekickListPtr.size(); i++)
		if (_ValidTargetFlag(actInst->SidekickListPtr[i], abilityRestrict)
			...
			if (actInst->SidekickListPtr[i]->IsSelfNearPoint(x, z, distance)
					== true)
				results.push_back(actInst->SidekickListPtr[i]);
}

//Add all targets within a certain distance of the player (AKA caster).
void CreatureInstance::AddTargetsPBAE(CREATURE_PTR_SEARCH &results,
		int abilityRestrict, int distance) {
	size_t i;
	for (i = 0; i < actInst->NPCListPtr.size(); i++)
		if (_ValidTargetFlag(actInst->NPCListPtr[i], abilityRestrict) == true)
...
void CreatureInstance::AddTargetsSTAE(CREATURE_PTR_SEARCH &results,
		int abilityRestrict, int distance) {
	if (CurrentTarget.targ == NULL)
		return;
	results.push_back(CurrentTarget.targ);
...
//current target.
void CreatureInstance::AddTargetsSTXAE(CREATURE_PTR_SEARCH &results,
		int abilityRestrict, int distance) {
	if (CurrentTarget.targ == NULL)
		return;
	...
	for (i = 0; i < actInst->NPCListPtr.size(); i++)
		if (actInst->NPCListPtr[i] != CurrentTarget.targ)
			if (_ValidTargetFlag(actInst->NPCListPtr[i], abilityRestrict)
					== true)
				if (IsObjectInRange(actInst->NPCListPtr[i], distance) == true)
//				if(actInst->GetPlaneRange(CurrentTarget.targ, actInst->NPCListPtr[i], distance) <= distance)
					results.push_back(actInst->NPCListPtr[i]);
	...
	for (i = 0; i < actInst->PlayerListPtr.size(); i++)
		if (actInst->PlayerListPtr[i] != CurrentTarget.targ)
			if (_ValidTargetFlag(actInst->PlayerListPtr[i], abilityRestrict)
					== true)
				if (IsObjectInRange(actInst->PlayerListPtr[i], distance)
						== true)
//				if(actInst->GetPlaneRange(CurrentTarget.targ, actInst->PlayerListPtr[i], distance) <= distance)
					results.push_back(actInst->PlayerListPtr[i]);
	...
	for (i = 0; i < actInst->SidekickListPtr.size(); i++)
		if (actInst->SidekickListPtr[i] != CurrentTarget.targ)
			if (_ValidTargetFlag(actInst->SidekickListPtr[i], abilityRestrict)
					== true)
				if (IsObjectInRange(actInst->SidekickListPtr[i], distance)
						== true)
//				if(actInst->GetPlaneRange(CurrentTarget.targ, actInst->SidekickListPtr[i], distance) <= distance)
					results.push_back(actInst->SidekickListPtr[i]);
}
...
void CreatureInstance::AddTargetsParty(CREATURE_PTR_SEARCH &results,
		int abilityRestrict, int distance) {
	size_t i;
	if (serverFlags & ServerFlags::IsPlayer) {
		...
		if (myParty != NULL) {
			for (i = 0; i < actInst->PlayerListPtr.size(); i++) {
				...
				if (CDefID != CreatureDefID) //Skip the caster, we'll add it later.
					...
					if (IsObjectInRange(actInst->PlayerListPtr[i], distance)
							== true) {
						ActiveParty *thatParty =
								g_PartyManager.GetPartyWithMember(CDefID);
...
void CreatureInstance::AddTargetsSTP(CREATURE_PTR_SEARCH &results,
		int abilityRestrict) {
...
int CreatureInstance::TransferTargets(const CREATURE_PTR_SEARCH &results,
		ActiveAbilityInfo &ab) {
...
int CreatureInstance::RequestAbilityActivation(int abilityID) {
	int result = actInst->ActivateAbility(this, abilityID, EventType::onRequest,
			NULL);
	if (result != Ability2::ABILITY_SUCCESS) {
		...
		if ((serverFlags & ServerFlags::IsPlayer) && (simulatorPtr != NULL)) {
			int errorCode = Ability2::AbilityManager2::GetAbilityErrorCode(
					result);
			g_Logs.simulator->debug(
					"Failed to activate ability %v because error code %v (%v).",
					abilityID, errorCode, result);
...
void CreatureInstance::ProcessAbility_Ex(ActiveAbilityInfo *ability) {
	...
	case AbilityType::Instant:
		r = actInst->ActivateAbility(this, ability->abilityID,
				EventType::onActivate, ability);
		...
		if (ability->type == AbilityType::Instant) {
			if (r == 0) {
				if (ability->x != 0.0F)
					ground = true;
				if (serverFlags & ServerFlags::IsPlayer) {
					size = PrepExt_AbilityActivateEmpty(&GSendBuf[size], this,
							ability, AbilityStatus::REQUEST_ACCEPTED);
					size += PrepExt_AbilityActivateEmpty(&GSendBuf[size], this,
							ability, AbilityStatus::ABILITY_FINISHED);
				}
				size += PrepExt_AbilityActivate(&GSendBuf[size], this, ability,
						AbilityStatus::ACTIVATE, ground);
				if (ability->x != 0.0F) {
					ability->x = 0.0F;
					ability->y = 0.0F;
					ability->z = 0.0F;
...
bool CreatureInstance::HasCooldown(int cooldownCategory) {
	...
	if ((r >= 0) && (serverFlags & ServerFlags::IsPlayer)) {
		if (simulatorPtr->CheckPermissionSimple(Perm_Account,
				Permission_SelfDiag)) {
			char buffer[64];
			Util::SafeFormat(buffer, sizeof(buffer),
					"Cooldown %d is active (%lu ms remain)", cooldownCategory,
					cooldownManager.cooldownList[r].GetRemainTimeMS());
...
void CreatureInstance::ForceAbilityActivate(int abilityID, int abilityEvent,
		int targetID) {
...
void CreatureInstance::SimulateEffect(const char *effect,
...
void CreatureInstance::SendAutoAttack(int abilityID, int targetID) {
	...
	if (serverFlags & ServerFlags::IsPlayer) {
		wpos += PrepExt_AbilityEvent(&GSendBuf[wpos], CreatureID, abilityID,
				AbilityStatus::REQUEST_ACCEPTED);
		wpos += PrepExt_AbilityEvent(&GSendBuf[wpos], CreatureID, abilityID,
				AbilityStatus::ABILITY_FINISHED);
	}
...
void CreatureInstance::ActivateSavedAbilities(void) {
	...
	if (g_Config.PersistentBuffs) {
		if (buffManager.buffList.size() > 0)
			...
			g_Logs.server->debug("No active buffs, %v persistent buffs",
					buffManager.persistentBuffList.size());
		...
		for (it = buffManager.persistentBuffList.begin();
				it != buffManager.persistentBuffList.end(); ++it) {
		...
		if (CurrentTarget.targ == NULL
			...
			if (spawnGen != NULL && spawnGen->spawnPoint != NULL
				...
				NPCDialogItem *diag = g_NPCDialogManager.GetItem(
						spawnGen->spawnPoint->extraData.dialog);
				if (diag != NULL && diag->mParagraphs.size() > 0) {
					...
					if (dialogIndex == -1) {
						...
						case ParagraphType::SAY:
							actInst->LSendToAllSimulator(GSendBuf,
									PrepExt_GenericChatMessage(GSendBuf,
											CreatureID, css.display_name, "s/",
											para.mValue.c_str()), -1);
							break;
						...
						case ParagraphType::EMOTE:
							actInst->LSendToAllSimulator(GSendBuf,
									PrepExt_GenericChatMessage(GSendBuf,
											CreatureID, css.display_name,
											"emote", para.mValue.c_str()), -1);
							break;
						}
					...
					g_Logs.simulator->warn(
							"Request for unknown or empty NPC dialog %v",
							spawnGen->spawnPoint->extraData.dialog);
					/* Never bother again */
					timer_dialog = -1;
				}
	...
	if (Speed == 0) {
		if (HasStatus(StatusEffects::WALK_IN_SHADOWS)
			...
			if (HasStatus(StatusEffects::INVISIBLE)) //Mage invisibility uses psyche
				dexBasedRegen = false;
			float newDist = css.invisibility_distance
					+ GetRegenInvisibilityDistance(dexBasedRegen);
...
void CreatureInstance::ProcessAutoAttack(void) {
	...
	if (css.melee_attack_speed != 0)
		nextDelay -=
				(int) (2000.0F * ((float) css.melee_attack_speed / 1000.0F));
	if (nextDelay < 500)
	...
	if (HasStatus(StatusEffects::AUTO_ATTACK_RANGED))
		atRes = CallAbilityEvent(ABILITYID_AUTO_ATTACK_RANGED,
...
void CreatureInstance::CheckMostHatedTarget(bool forceCheck) {
	...
	for (size_t i = 0; i < hateProfilePtr->hateList.size(); i++) {
		...
		int dist = ActiveInstance::GetPlaneRange(this, targ,
				AGGRO_MAXIMUM_RANGE);
		if (dist <= AGGRO_MAXIMUM_RANGE) {
			...
			if (serverFlags & ServerFlags::Taunted) {
				...
				if (dist > TAUNT_BECKON_DISTANCE)
					MoveTo(targ->CurrentX, targ->CurrentZ,
							TAUNT_BECKON_DISTANCE, 0);
			}
			return;
		}
	}
...
void CreatureInstance::RunAutoTargetSelection(void) {
	...
	if ((serverFlags & ServerFlags::NeutralInactive)
			|| (serverFlags & ServerFlags::Stationary)
			|| (serverFlags & ServerFlags::Noncombatant))
		return;
...
void CreatureInstance::RunProcessingCycle(void) {
	...
	if (css.health > 0 && NotStatus(StatusEffects::DEAD)) {
		...
		if (serverFlags & ServerFlags::IsNPC) {
			...
			if (charPtr->inventory.NextExpunge != 0
					&& g_ServerTime >= charPtr->inventory.NextExpunge) {
				int r = charPtr->inventory.RemoveExpiredItemsAndUpdate(
						GSendBuf);
				if (r > 0) {
					charPtr->pendingChanges++;
			...
			if (CurrentTarget.targ == NULL) {
				...
				if ((!(serverFlags & ServerFlags::CalledBack))
						&& (!(serverFlags & ServerFlags::Noncombatant)))
...
void CreatureInstance::CheckPathLocation(void) {
	...
	SceneryObject *so = g_SceneryManager.GlobalGetPropPtr(actInst->mZone,
			nextPathNode, NULL);
	if (so == NULL) {
	...
	if (so->extraData.link.size() == 0) {
		g_Logs.server->warn("CheckPathLocation() no links on prop: %v",
				nextPathNode);
		return;
	}
	...
	if (newPathNode != nextPathNode)
		newWaypoint = g_SceneryManager.GlobalGetPropPtr(actInst->mZone,
				newPathNode, NULL);
...
int CreatureInstance::RunMovementStep(void) {
	...
	if (dist < closeEnough) {
		...
		if (distRemain > 0) {
			...
			float distPerSecond = ((float) Speed / 100.0F)
					* DEFAULT_CREATURE_SPEED;

			//The number of units to move for this update.
			int updateDist = (int) (distPerSecond
					* ((float) nextMoveTime / 1000.0F));
...
void CreatureInstance::UpdateDestination(void) {
	...
	if (CurrentTarget.targ == NULL) {
		if (serverFlags & ServerFlags::IsNPC) {
			...
			//is being followed.
			if ((serverFlags & ServerFlags::PathNode)
					&& !(serverFlags & ServerFlags::LeashRecall)) {
...
void CreatureInstance::CheckLeashMovement(void) {
	...
	if (serverFlags & ServerFlags::ScriptMovement) {
		...
		if ((CurrentTarget.targ == NULL)
				&& (CurrentTarget.DesLocX != 0 || CurrentTarget.DesLocZ != 0))
			return;
	}
	...
	if (maxLeash > 0) {
		...
		if ((xlen > maxLeash) || (zlen > maxLeash)) {
			...
			css.health = CalcRestrictedHealth(-1, true);
			int size = PrepExt_SendHealth(GSendBuf, this->CreatureID,
					css.health);
			actInst->LSendToLocalSimulator(GSendBuf, size, CurrentX, CurrentZ);
		} else {
			...
			if (CurrentTarget.targ == NULL && CurrentTarget.DesLocX == 0
	...
	if (scriptMoveEvent != -1) {
		if (actInst != NULL && actInst->nutScriptPlayer != NULL) {
			ScriptCore::NutScriptEvent *nse =
					actInst->nutScriptPlayer->GetEvent(scriptMoveEvent);
			if (nse != NULL) {
...
//	static const int YOFFSET = 50;
	...
	if (CurrentTarget.DesLocX == 0 || CurrentTarget.desiredRange != 0) {
		if (CurrentTarget.targ == NULL) {
			...
			int dist = actInst->GetPlaneRange(this, AnchorObject,
					SANE_DISTANCE);
			...
			if (CurrentTarget.desiredRange != 0) {
				...
				if (dist > CurrentTarget.desiredRange) {
					...
					CurrentTarget.DesLocZ = CurrentTarget.targ->CurrentZ
							+ (int) ((float) (CurrentTarget.desiredRange - 5)
									* cos(rotf));
					CurrentTarget.DesLocX = CurrentTarget.targ->CurrentX
							+ (int) ((float) (CurrentTarget.desiredRange - 5)
									* sin(rotf));
...
void CreatureInstance::SelectTarget(CreatureInstance *newTarget) {
	if (newTarget != CurrentTarget.targ) {
		...
		if (CurrentTarget.targ != NULL && aiNut != NULL) {
			 ...
			 */
			g_Logs.server->debug("%v lost their target (which was %v).",
...
void CreatureInstance::AICheckAbilityFailure(int abilityReturnInfo) {
	...
	case Ability2::ABILITY_NO_TARGET: //For some reason swarm isn't being updated with a ground location
		if (CurrentTarget.targ != NULL) {
			if (ActiveInstance::GetPlaneRange(this, CurrentTarget.targ,
	...
	case Ability2::ABILITY_INRANGE:
		...
		CurrentTarget.desiredRange = g_AbilityManager.GetAbilityErrorParameter(
				abilityReturnInfo);
		break;
	}
}
...
int CreatureInstance::AIFillEnemyNear(int range, float x, float z,
		CREATURE_PTR_SEARCH& enemies) {
	for (size_t i = 0; i < actInst->PlayerListPtr.size(); i++) {
		if (ActiveInstance::GetPointRangeXZ(actInst->PlayerListPtr[i], x, z,
				range) > range)
			continue;
		if (_ValidTargetFlag(actInst->PlayerListPtr[i],
				TargetStatus::Enemy_Alive) == true)
			enemies.push_back(actInst->PlayerListPtr[i]);
	}
	for (size_t i = 0; i < actInst->NPCListPtr.size(); i++) {
		if (ActiveInstance::GetPointRangeXZ(actInst->NPCListPtr[i], x, z, range)
				> range)
			continue;
		if (_ValidTargetFlag(actInst->NPCListPtr[i], TargetStatus::Enemy_Alive)
				== true)
			enemies.push_back(actInst->NPCListPtr[i]);
	}
	for (size_t i = 0; i < actInst->SidekickListPtr.size(); i++) {
		if (ActiveInstance::GetPointRangeXZ(actInst->SidekickListPtr[i], x, z,
				range) > range)
			continue;
		if (_ValidTargetFlag(actInst->SidekickListPtr[i],
				TargetStatus::Enemy_Alive) == true)
			enemies.push_back(actInst->NPCListPtr[i]);
	}
	return enemies.size();
...
int CreatureInstance::AIFillCreaturesNear(int range, float x, float z,
		int playerAbilityRestrict, int npcAbilityRestrict,
		int sidekickAbilityRestrict, CREATURE_PTR_SEARCH& creatures) {
	for (size_t i = 0; i < actInst->PlayerListPtr.size(); i++) {
		...
		if (ActiveInstance::GetPointRangeXZ(actInst->PlayerListPtr[i], x, z,
				range) > range)
			continue;
		if (_ValidTargetFlag(actInst->PlayerListPtr[i], playerAbilityRestrict)
				== true)
	...
	for (size_t i = 0; i < actInst->NPCListPtr.size(); i++) {
		...
		if (ActiveInstance::GetPointRangeXZ(actInst->NPCListPtr[i], x, z, range)
				> range)
			continue;
		if (_ValidTargetFlag(actInst->NPCListPtr[i], npcAbilityRestrict)
				== true)
...
void CreatureInstance::AIOtherCallLabel(int creatureID,
		const char *aiScriptLabel) {
	CreatureInstance *obj = actInst->GetNPCInstanceByCID(creatureID);
	if (obj != NULL) {
		if (obj->aiNut != NULL) {
...
void CreatureInstance::SendEffect(const char *effectName, int targetCID) {
	if (actInst != NULL) {
		char buffer[512];
		int wpos = PrepExt_SendEffect(buffer, CreatureID, effectName,
				targetCID);
...
void CreatureInstance::SendSay(const char *message) {
	if (actInst != NULL) {
		char buffer[512];
		int wpos = PrepExt_GenericChatMessage(buffer, CreatureID,
				css.display_name, "s", message);
		BroadcastLocal(buffer, wpos, -1);
	}
}

void CreatureInstance::SendPlaySound(const char *assetPackage,
...
void CreatureInstance::_AddStatusList(int statusID, long msDuration) {
	...
	if (r >= 0) {
		...
		activeStatusEffect.push_back(
				ActiveStatusEffect(statusID, g_ServerTime, endTime));
...
void CreatureInstance::CheckLootTimers(void) {
	...
	if (activeLootID != 0) {
		...
		for (size_t i = 0; i < actInst->lootsys.creatureList[r].itemList.size();
				i++) {
			int itemID = actInst->lootsys.creatureList[r].itemList[i];
			ItemDef *itemPtr = g_ItemManager.GetPointerByID(itemID);
			if (itemPtr != NULL) {
				if (itemPtr->mType == ItemType::ARMOR
						|| itemPtr->mType == ItemType::WEAPON) {
	...
	static const short appStat[2] = { STAT::APPEARANCE_OVERRIDE,
...
int CreatureInstance::GetKillExperience(int attackerLevel) {
	...
	if (css.experience_gain_rate > 0)
		adjusted += Util::GetAdditiveFromIntegralPercent100(adjusted,
				css.experience_gain_rate);
	if (adjusted < 1)
		adjusted = 1;
...
void CreatureInstance::AddValour(int GuildDefID, int amount) {
	...
	GuildRankObject *currentRank = g_GuildManager.GetRank(CreatureDefID,
			GuildDefID);
	g_CharacterManager.GetThread("CharacterData::AddValour");
	charPtr->AddValour(GuildDefID, amount);
	g_CharacterManager.ReleaseThread();
	GuildRankObject *newRank = g_GuildManager.GetRank(CreatureDefID,
			GuildDefID);
	if (newRank == NULL) {
		g_Logs.server->error("No new rank in guild, joined %v to %v",
				CreatureDefID, GuildDefID);
		return;
	}
	if (currentRank == NULL || currentRank->valour != newRank->valour) {
		char buffer[128];
		Util::SafeFormat(buffer, sizeof(buffer),
				"You have gained %d valour in %s", amount,
				guildDef->defName.c_str());
		simulatorPtr->SendInfoMessage(buffer, INFOMSG_INFO);
	}
	if (currentRank == NULL || currentRank->rank != newRank->rank) {
		...
		char buffer[128];
		Util::SafeFormat(buffer, sizeof(buffer),
				"You have been promoted to %s in %s", newRank->title.c_str(),
				guildDef->defName.c_str());
		simulatorPtr->SendInfoMessage(buffer, INFOMSG_INFO);
...
void CreatureInstance::AddExperience(int amount) {
	...
	if (serverFlags & ServerFlags::IsPlayer) {
		...
		wpos += PrepExt_SendExperience(&GSendBuf[wpos], CreatureID,
				css.experience);
		actInst->LSendToOneSimulator(GSendBuf, wpos, simulatorPtr);
...
void CreatureInstance::AddHeroism(int amount) {
	...
	int newHeroism = Util::ClipInt(css.heroism + amount, 0,
			Global::MAX_HEROISM);
...
void CreatureInstance::AddHeroismForQuest(int amount, int questLevel) {
	...
	if (css.heroism_gain_rate > 0) {
		amount += Util::GetAdditiveFromIntegralPercent100(amount,
				css.heroism_gain_rate);
	}
...
void CreatureInstance::AddHeroismForKill(int targetLevel, int targetRarity) {
	...
	// Adjust for other bonuses
	if (css.heroism_gain_rate > 0) {
		points += Util::GetAdditiveFromIntegralPercent100(points,
				css.heroism_gain_rate);
	}
...
void CreatureInstance::OnHeroismChange(void) {
	...
	css.health_mod = Util::GetAdditiveFromIntegralPercent1000(
			GetMaxHealth(false), modPercent);
...
void CreatureInstance::CheckQuestKill(CreatureInstance *target) {
	...
	int wpos = charPtr->questJournal.CheckQuestObjective(CreatureID, GSendBuf,
			QuestObjective::OBJECTIVE_TYPE_KILL, target->CreatureDefID,
			target->CreatureID);
	if (wpos > 0) {
		//TODO: Simulator revamp
		if (simulatorPtr != NULL)
...
int CreatureInstance::ProcessQuestRewards(int QuestID, int outcomeIdx,
	...
	if (qd == NULL) {
		g_Logs.server->error("ProcessQuestRewards() invalid quest ID [%v]",
				QuestID);
		return -1;
	}
	...
	if (outcome == NULL) {
		g_Logs.server->error(
				"ProcessQuestRewards() invalid quest ID [%v] has no outcome for index %v",
				QuestID, outcomeIdx);
		return -1;
	}
	...
	for (size_t i = 0; i < itemsToGive.size(); i++) {
		...
		ItemDef *itemPtr = g_ItemManager.GetPointerByID(itemID);
		if (itemPtr == NULL) {
			wpos += PrepExt_SendInfoMessage(&GSendBuf[wpos],
					"Reward item not found in database.", INFOMSG_ERROR);
			simulatorPtr->AttemptSend(GSendBuf, wpos);
			g_Logs.server->error(
					"ProcessQuestRewards() Reward item [%v] not found in database for quest [%v]",
					itemID, QuestID);
			return -1;
		}
		...
		if (slot == -1) {
			wpos += PrepExt_SendInfoMessage(&GSendBuf[wpos],
					"No free inventory space.", INFOMSG_ERROR);
			simulatorPtr->AttemptSend(GSendBuf, wpos);
			return -2;
		}

		InventorySlot *newItem = NULL;
		if (itemPtr->mType == ItemType::SPECIAL
				&& itemPtr->mIvType1 == ItemIntegerType::QUEST_ID) {
			...
			if (iid != -1) {
				newItem = charPtr->inventory.AddItem_Ex(INV_CONTAINER, iid,
						count);
				if (newItem != NULL) {
					char buffer[256];
					Util::SafeFormat(buffer, sizeof(buffer),
							"Your mystery reward is .. %s",
							newItem->dataPtr->mDisplayName.c_str());
					wpos += PrepExt_SendInfoMessage(&GSendBuf[wpos], buffer,
							INFOMSG_INFO);
				}
			}
		} else {
			newItem = charPtr->inventory.AddItem_Ex(INV_CONTAINER, itemPtr->mID,
					count);
		}
		if (newItem != NULL) {
			charPtr->pendingChanges++;
			...
			wpos += PrepExt_SendInfoMessage(&GSendBuf[wpos], "No item.",
					INFOMSG_ERROR);
			simulatorPtr->AttemptSend(GSendBuf, wpos);
			return -2;
		}
...
int CreatureInstance::QuestInteractObject(char *buffer, const char *text,
		int time, bool gather) {
	Interrupt();
	return PrepInteractObject(buffer, text, time, gather, CurrentTarget.targ);
}

int CreatureInstance::PrepInteractObject(char *buffer, const char *text,
		int time, bool gather, CreatureInstance *targ) {
	g_Logs.server->debug("PrepInteractObject %v : %v : %v", text, time,
			gather ? "gather: " : "activate");
...
int CreatureInstance::NormalInteractObject(char *outBuf,
...
void CreatureInstance::CheckQuestInteract(CreatureInstance *target) {
		...
		for (std::vector<int>::iterator it = cdef->Items.begin();
			...
			if (charPtr->inventory.GetItemPtrByID(id) == NULL) {
				...
				if (item->mID == 0) {
					...
					char buf2[256];
					if (slot == -1) {
						Util::SafeFormat(buf2, sizeof(buf2),
								"Cannot take book, your inventory is full.",
								item->mDisplayName.c_str());
						int wpos = PrepExt_SendInfoMessage(buf, buf2,
								INFOMSG_INFO);
						simulatorPtr->AttemptSend(buf, wpos);
						return;
					} else {
						InventorySlot *sendSlot = charPtr->inventory.AddItem_Ex(
						...
						if (sendSlot != NULL) {
							...
							Util::SafeFormat(buf2, sizeof(buf2),
									"You now have '%s' in your inventory.",
									item->mDisplayName.c_str());
							wpos += PrepExt_SendInfoMessage(&buf[wpos], buf2,
									INFOMSG_INFO);
							simulatorPtr->AttemptSend(buf, wpos);
							...
							static const short statList[3] = {
									...
									STAT::LOOT_SEEABLE_PLAYER_IDS };
							actInst->LSendToLocalSimulator(GSendBuf,
									PrepExt_SendSpecificStats(GSendBuf, target,
											&statList[0], 3), target->CurrentX,
									target->CurrentZ);
	...
	int wpos = charPtr->questJournal.CreatureUse_Confirmed(CreatureID, GSendBuf,
			target->CreatureDefID, target->CreatureID);
	if (wpos > 0) {
		simulatorPtr->AttemptSend(GSendBuf, wpos);
		...
		QuestScript::QuestScriptPlayer *script =
				actInst->GetSimulatorQuestScript(simulatorPtr);
		if (script != NULL)
			script->TriggerFinished();
	}
}

void CreatureInstance::RunQuestObjectInteraction(CreatureInstance *target,
		bool deleteObject) {
	if (target == NULL)
		return;
	if (!(serverFlags & ServerFlags::IsPlayer))
	 ...
	 */
	InteractObject *ob = g_InteractObjectContainer.GetObjectByID(
			target->CreatureDefID, target->actInst->mZone);
		...
		if (party != NULL) {
			for (size_t i = 0; i < party->mMemberList.size(); i++) {
				...
				if (CDefID != CreatureDefID) {
					CreatureInstance *member = actInst->GetPlayerByCDefID(
							CDefID);
					if (member == NULL)
						continue;
					if (member->actInst->mInstanceID != instance)
						continue;
					if (actInst->GetPlaneRange(this, member,
							PARTY_SHARE_DISTANCE) >= PARTY_SHARE_DISTANCE)
						continue;
...
void CreatureInstance::SendUpdatedLoot(void) {
	static const short statList[3] = { STAT::APPEARANCE_OVERRIDE,
			STAT::LOOTABLE_PLAYER_IDS, STAT::LOOT_SEEABLE_PLAYER_IDS, };

	if (activeLootID != 0) {
		ActiveLootContainer &lootCon =
				actInst->lootsys.creatureList[activeLootID];
		char convBuf[32];
		int wpos = 0;
		...
		for (size_t i = 0; i < lootCon.lootableID.size(); i++) {
			Util::SafeFormat(convBuf, sizeof(convBuf), "%d",
					lootCon.lootableID[i]);
			if (i > 0) {
...
float CreatureInstance::GetDropRateMultiplier(CreatureDefinition *cdef) {
	...
	if (css.drop_gain_rate > 0) {
		dropRateBonus += Util::GetAdditiveFromIntegralPercent100(dropRateBonus,
				css.drop_gain_rate);
	}
	...
	if (dropRateBonus > 2.0F)
		g_Logs.server->debug("DropRateMult [%v] %v,%v: %v", css.display_name,
				css.level, css.rarity, dropRateBonus);
...
//				params.SetAllDropMult(dropRateBonus);
			...
			params.dropRateProfile = g_DropRateProfileManager.GetProfileByName(
					profile.virtualItemReward.dropRateProfileName);
			params.ClampLimits();
		...
		case RewardType::ITEM:
			...
			if (item == NULL) {
				g_Logs.server->warn(
						"An item (%v) exists in the daily loot table that does not exist.",
						itemID);
			}
			break;
		}
...
void CreatureInstance::CreateLoot(int finderLevel, int partySize) {
	...
	if (cdef->DropRateProfile.length() == 0 && actInst != NULL)
		dropRateProfile.CopyFrom(
				g_DropRateProfileManager.GetProfileByName(
						actInst->mZoneDefPtr->GetDropRateProfile()));
	else
		dropRateProfile.CopyFrom(
				g_DropRateProfileManager.GetProfileByName(
						cdef->DropRateProfile));
...
void CreatureInstance::SetLevel(int newLevel) {
	...
	css.level = newLevel;
	g_Logs.event->info("[PLAYER] Player [%v] had reached level [%v]",
	...
	char sbuffer[256];
	Util::SafeFormat(sbuffer, sizeof(sbuffer), "%s is now level %d!",
			css.display_name, css.level);
...
int CreatureInstance::GetAggroRange(CreatureInstance *target) {
	...
	if (target->HasStatus(StatusEffects::WALK_IN_SHADOWS)
			|| target->HasStatus(StatusEffects::INVISIBLE)) {
		if (isTargetInCone(target, Cone_90) == false
				&& css.rarity <= CreatureRarityType::HEROIC) {
...
bool CreatureInstance::CAF_Untransform() {
	...
	g_Logs.server->debug("Untransforming %v (app is %v)", CreatureDefID,
			css.appearance.c_str());
...
bool CreatureInstance::CAF_Nudify(int durationS) {
	...
	g_Logs.server->debug("Nudifying %v (app is %v)", CreatureDefID,
			css.appearance.c_str());
...
bool CreatureInstance::CAF_Scale(float scale, int durationS) {
	...
	g_Logs.server->debug("Scaling %v (app is %v) to %v", CreatureDefID,
			css.appearance.c_str(), scale);
	char buf[64];
...
bool CreatureInstance::CAF_Transform(int CDefID, int abID, int durationS) {
	if (transformModifier != NULL) {
		g_Logs.server->warn("%v already transformed into %v", CreatureDefID,
				transformCreatureId);
		return false;
	}
...
int CreatureInstance::CAF_SummonSidekick(int CDefID, int maxSummon,
		short abGroupID) {
	...
	if (count >= maxSummon) {
		int wpos = PrepExt_SendInfoMessage(GSendBuf,
				"You cannot summon any more creatures of that type.",
				INFOMSG_INFO);
		simulatorPtr->AttemptSend(GSendBuf, wpos);
		return -1;
	}
...
void CreatureInstance::StatScaleToLevel(int statID, int targetLevel) {
	...
	g_Logs.server->debug("Level %v to %v, value: %v, mult: %v, result: %v",
			css.level, targetLevel, value, mult, value * mult);
...
int CreatureInstance::CAF_RegisterTargetSidekick(int abGroupID) {
	...
	for (size_t i = 0; i < charPtr->SidekickList.size(); i++) {
		...
		for (size_t s = search; s < actInst->SidekickListPtr.size(); s++)
			if (actInst->SidekickListPtr[s]->AnchorObject == this)
				if (actInst->SidekickListPtr[s]->CreatureDefID
						== charPtr->SidekickList[i].CDefID) {
	...
	for (int i = 0; i < count; i++)
		g_Logs.server->debug("[%v] = [%v]", i,
				ab[0].TargetList[i]->css.display_name);
...
void CreatureInstance::DetachItem(const char *type, const char *node) {
	for (unsigned int i = 0; i < appearanceModifiers.size(); i++) {
		AddAttachmentModifier *mod =
				dynamic_cast<AddAttachmentModifier*>(appearanceModifiers[i]);
...
void CreatureInstance::PushAppearanceModifier(AppearanceModifier *modifier) {
	g_Logs.server->debug(
			"Pushing an appearance modifier to %v in a list of %v, now %v",
			CreatureID, appearanceModifiers.size(),
			appearanceModifiers.size() + 1);
...
int CreatureInstance::GetOffhandDamage(void) {
	...
	if (EquippedWeapons[ItemEquipSlot::WEAPON_OFF_HAND] == 0
			|| OffhandDamage[1] == 0)
		return 0;
...
void CreatureInstance::Respec(void) {
	...
	static const int EffectList[] = { StatusEffects::CAN_USE_WEAPON_2H,
			...
			StatusEffects::CAN_USE_DUAL_WIELD, StatusEffects::CAN_USE_PARRY,
			StatusEffects::CAN_USE_BLOCK, StatusEffects::CAN_USE_ARMOR_CLOTH,
			StatusEffects::CAN_USE_ARMOR_LIGHT,
			StatusEffects::CAN_USE_ARMOR_MEDIUM,
			StatusEffects::CAN_USE_ARMOR_HEAVY, StatusEffects::INVISIBLE,
...
void CreatureInstance::OnEquipmentChange(float oldHealthRatio) {
	...
	css.health = CalcRestrictedHealth(Util::ClipInt(newHealth, 0, newMax),
			true);
	SendStatUpdate(STAT::HEALTH);
	OnHeroismChange();
}
...
int CreatureInstance::GetLevelScaledValue(int originalAmount, int targetLevel,
		float multPerLevel) {
	int levelDiff = targetLevel - css.level;
	if (levelDiff < 1)
		levelDiff = 1;
...
void CreatureInstance::PerformLevelScale(
		const InstanceScaleProfile *scaleProfile, int targetLevel) {
	...
	if (scaleProfile->mStatMultBonus > 0.0F) {
		css.strength = GetScaledValue(css.strength,
				scaleProfile->mStatMultBonus);
		css.dexterity = GetScaledValue(css.dexterity,
				scaleProfile->mStatMultBonus);
	...
	if (scaleProfile->mConMultBonus > 0.0F) {
		css.constitution = GetScaledValue(css.constitution,
				scaleProfile->mConMultBonus);
	}

	if (scaleProfile->mArmorMultBonus > 0.0F) {
		css.damage_resist_melee = GetScaledValue(css.damage_resist_melee,
				scaleProfile->mArmorMultBonus);
		css.damage_resist_fire = GetScaledValue(css.damage_resist_fire,
				scaleProfile->mArmorMultBonus);
		css.damage_resist_frost = GetScaledValue(css.damage_resist_frost,
				scaleProfile->mArmorMultBonus);
		css.damage_resist_mystic = GetScaledValue(css.damage_resist_mystic,
				scaleProfile->mArmorMultBonus);
		css.damage_resist_death = GetScaledValue(css.damage_resist_death,
				scaleProfile->mArmorMultBonus);
	}

	if (scaleProfile->mDmgMultBonus > 0.0F) {
		css.base_damage_melee = GetScaledValue(css.base_damage_melee,
				scaleProfile->mDmgMultBonus);
	}
...
void CreatureInstance::BuildZoneString(int instanceID, int zoneID,
		int unknownID) {
	Util::SafeFormat(ZoneString, sizeof(ZoneString), "[%d-%d-%d]", instanceID,
			zoneID, unknownID);
}
...
void CreatureInstance::OnInstanceEnter(const ArenaRuleset &arenaRuleset) {
	...
	if (arenaRuleset.mPVPStatus == PVP::GameMode::SPECIAL_EVENT
			|| arenaRuleset.mPVPStatus == PVP::GameMode::PVP_ONLY
			|| (arenaRuleset.mPVPStatus != PVP::GameMode::PVE_ONLY
					&& charPtr->Mode == PVP::GameMode::PVP)) {
	...
	for (size_t i = 0; i < arenaRuleset.mRuleList.size(); i++) {
		...
		if (pass == true) {
			...
			AddBuff(BuffSource::INSTANCE, 0, 0, 0, 0, StatID, additive,
					additive, -1);
		} else {
...
void CreatureInstance::BroadcastLocal(const char *buffer, int dataLen,
		int simulatorFilter) {
	if (actInst == NULL)
		return;
	actInst->LSendToLocalSimulator(buffer, dataLen, CurrentX, CurrentZ,
			simulatorFilter);
}
...
bool CreatureInstance::IsObjectInRange(CreatureInstance *object,
		float distance) {
	if (object == NULL)
		return false;
	...
	//Don't include Y axis since the server doesn't know about elevation differences.
	float objDist = ActiveInstance::GetPlaneRange(this, object,
			static_cast<int>(acceptDist));
...
int CreatureInstance::GetDistance(CreatureInstance *object, int threshold) {
	...
	//Don't include Y axis since the server doesn't know about elevation differences.
	return ActiveInstance::GetPlaneRange(this, object,
			static_cast<int>(acceptDist));
}
...
bool CreatureInstance::IsSelfNearPoint(float x, float z, float distance) {
	...
	float acceptDist = tolerance + distance;
	float dist = ActiveInstance::GetPointRangeXZ(this, x, z,
			static_cast<int>(acceptDist));
	if (dist <= acceptDist)
		return true;
...
void CreatureInstance::DebugGenerateReport(ReportBuffer &report) {
	...
	for (i = 0; i < NumStats; i++)
		if (isStatZero(i, &css) == false)
			report.AddLine("%s=%s", StatList[i].name,
					GetStatValueAsString(i, ConvBuf, &css));
	...
	for (i = 0; i < baseStats.size(); i++) {
		...
		if (r >= 0)
			report.AddLine("%d = %s (%d mods): %g + %g = %g",
					baseStats[i].StatID, StatList[r].name,
					baseStats[i].RefCount, baseStats[i].fBaseVal,
	...
	for (i = 0; i < activeStatusEffect.size(); i++) {
		...
		if (activeStatusEffect[i].expireTime == PlatformTime::MAX_TIME)
			report.AddLine("%hd = infinite (%s)",
					activeStatusEffect[i].modStatusID,
					GetStatusNameByID(statID).c_str());
		else
			report.AddLine("%hd = %lu (%s)", activeStatusEffect[i].modStatusID,
					extime, GetStatusNameByID(statID).c_str());
	}

	report.AddLine(NULL);
	report.AddLine("IMPLICIT ACTIONS");
	for (i = 0; i < implicitActions.size(); i++) {
		report.AddLine("[%d] = type:%d, abID:%d, abGroup:%d", i,
				implicitActions[i].eventType, implicitActions[i].abilityID,
...
void CreatureDefManager::AddNPCDef(CreatureDefinition &newItem) {
	...
	if (it == NPC.end())
		...
		g_Logs.server->warn("Overwriting Creature Def: %v",
				newItem.CreatureDefID);
	...
	while (lfr.FileOpen() == true) {
		...
		if (r > 0) {
			...
			if (strcmp(lfr.SecBuffer, "[ENTRY]") == 0) {
				...
				newItem.DropRateMult = str.GetValueFloatOrDefault(
						"dropratemult", 1);
				newItem.DropRateProfile = "";
				str.GetValueString("droprateprofile", newItem.DropRateProfile);
				for (MULTISTRING::iterator it = str.mData.begin();
						it != str.mData.end(); ++it) {
					STRINGLIST sl = *it;
					if (sl[0].compare("item") == 0) {
						newItem.Items.push_back(
								Util::GetInteger(sl[1].c_str()));
					}
				}
				...
				for (int a = 1; a < params; a++) {
					int r = GetStatusIDByName(
							lfr.BlockToStringC(a, Case_Upper));
					if (r >= 0)
						newItem.DefaultEffects.push_back(r);
					else
						g_Logs.data->warn(
								"Unknown status effect identifier [%v] in file [%v]",
								lfr.SecBuffer, filename);
				}
			} else {
				strncpy(StrBuf, lfr.BlockToStringC(0, 0), sizeof(StrBuf) - 1);
				r = WriteStatToSetByName(StrBuf, lfr.BlockToStringC(1, 0),
						&newItem.css);
				if (r == -1) {
...
void PendingOperation::UpdateList_Add(unsigned short mask,
		CreatureInstance *obj, short statID) {
	...
	for (size_t a = 0; a < UpdateList.size(); a++) {
		...
		if (UpdateList[a].mask & CREATURE_UPDATE_STAT) {
			if (UpdateList[a].statUpdate.size() > 0) {
				...
				wpos += PrepExt_SendSpecificStats(&GSendBuf[wpos], ptr,
						UpdateList[a].statUpdate);
				//wpos += PrepExt_SendInfoMessage(&GSendBuf[wpos], "UpdateList_Process after", INFOMSG_INFO);
				ptr->actInst->LSendToLocalSimulator(GSendBuf, wpos,
						ptr->CurrentX, ptr->CurrentZ);
...
int PrepExt_SendSpecificStats(char *buffer, CreatureInstance *cInst,
		vector<short> &statList) {
	//Prepares a buffer with the data necessary to update a Creature Definition.
	int wpos = 0;
	...
	for (int a = 0; a < size; a++) {
		...
		wpos += WriteCurrentStatToBuffer(&buffer[wpos], statList[a],
				&cInst->css);
	}
...
int PrepExt_SendSpecificStats(char *buffer, CreatureInstance *cInst,
		const short *statList, int statCount) {
	//Prepares a buffer with the data necessary to update a Creature Definition.
	int wpos = 0;
	...
	if (mask & CREATURE_UPDATE_MOD) {
		...
		for (a = 0; a < modCount; a++) {
			if (g_ProtocolVersion > 15) {
				...
				wpos += PutShort(&buffer[wpos],
						cInst->activeStatMod[a].modStatID);  //ID
				wpos += PutShort(&buffer[wpos],
						cInst->activeStatMod[a].abilityID);  //Ability ID

				if (priority == 1)
					wpos += PutFloat(&buffer[wpos],
							cInst->activeStatMod[a].clientAmount);
				else
					wpos += PutShort(&buffer[wpos],
							(short) cInst->activeStatMod[a].clientAmount);
				...
				wpos += PutShort(&buffer[wpos],
						cInst->activeStatMod[a].abilityID);  //Ability ID
				wpos +=
						PutShort(&buffer[wpos],
								static_cast<short>(cInst->activeStatMod[a].clientAmount)); //amount
			}
//			wpos += PutInteger(&buffer[wpos], (int)((cInst->activeStatMod[a].expireTime - g_ServerTime) / 1000));  //duration
		...
		for (a = 0; a < (int) cInst->activeStatusEffect.size(); a++) {
			wpos += PutShort(&buffer[wpos],
					cInst->activeStatusEffect[a].modStatusID);
		}
	}
	...
	if (mask & CREATURE_UPDATE_STAT) {
		...
		for (size_t i = 0; i < cInst->baseStats.size(); i++) {
			...
			wpos += WriteStatToBuffer(&buffer[wpos], cInst->baseStats[i].StatID,
					cInst->baseStats[i].fBaseVal
							+ cInst->baseStats[i].fModTotal);
			r++;
		}
		PutShort(&buffer[spos], r);           //Write number of stats
	}
...
int PrepExt_CreatureFullInstance(char *buffer, CreatureInstance *cInst) {
	...
	short mask = CREATURE_UPDATE_TYPE | CREATURE_UPDATE_STAT
			| CREATURE_UPDATE_MOD | CREATURE_UPDATE_ZONE
			| CREATURE_UPDATE_ELEVATION | CREATURE_UPDATE_VELOCITY;
	if (g_ProtocolVersion < 22)
		wpos += PutByte(&buffer[wpos], mask);
	else
		wpos += PutShort(&buffer[wpos], mask);
...
int PrepExt_GeneralMoveUpdate(char *buffer, CreatureInstance *cInst) {
	...
	wpos += PutInteger(&buffer[wpos], cInst->CreatureID);
	unsigned char mask = CREATURE_UPDATE_ELEVATION
			| CREATURE_UPDATE_POSITION_INC | CREATURE_UPDATE_VELOCITY;
	if (g_ProtocolVersion < 22)
		wpos += PutByte(&buffer[wpos], mask);
	else
		wpos += PutShort(&buffer[wpos], mask);
...
int PrepExt_AbilityActivate(char *buffer, CreatureInstance *cInst,
		ActiveAbilityInfo *ability, int aevent, bool ground) {
	int wpos = 0;
	...
	if (ability->bSecondary == false) {
		...
		for (a = 0; a < ability->TargetCount; a++) {
			if (ability->TargetList[a] == NULL)
				g_Logs.server->warn(
						"Ability %d for event %d has target count of %d, but target %d is NULL.",
						ability->abilityID, aevent, ability->TargetCount, a);
			else
				wpos += PutInteger(&buffer[wpos],
						ability->TargetList[a]->CreatureID);   //targets
		}
	...
	if (g_ProtocolVersion > 20) {
		if (aevent == AbilityStatus::ACTIVATE
				|| aevent == AbilityStatus::CHANNELING) {
...
int PrepExt_AbilityActivateEmpty(char *buffer, CreatureInstance *cInst,
		ActiveAbilityInfo *ability, int aevent) {
