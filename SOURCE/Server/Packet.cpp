...
{
	report.AddLine("%s|%lu|%lu|%lu", mErrorTimestamp.c_str(), mPartialEventCount, mPartialBytesTotal, mPendingBytesTotal);
}
...
void Packet :: Append(const Packet &data)
{
	if(data.mData.size() > 100000)
		g_Log.AddMessageFormat("[CRITICAL] Append(&data) bytecount: %d", data.mData.size());
...
void Packet :: TrimFront(size_t byteCount)
{
	...
	if(mData.size() > 100000)
		g_Log.AddMessageFormat("[CRITICAL] TrimFront mData.size: %d", mData.size());
...
void PacketManager :: ExternalAddPacket(int socket, const char *data, int length)
{
	if(length == 0)
	{
		g_Log.AddMessageFormat("Cannot add packet of zero size.");
...
void PacketManager :: SendPackets2(void)
{
	...
	while(it != mSendData.end())
	{
		...
		else
		{
			...
			if(res == SEND_FAILED)
			{
				g_Log.AddMessageFormat("[SOCKET] Disconnecting Socket:%d", it->mSocket);
...
int PacketManager :: SendSocket(int socket, PendingSocket &socketData)
{
	...
	while(pit != socketData.mPacketList.end())
	{
		...
		else
		{
			...
			if(derror.mCurrent.mPartialEventCount % g_Config.DebugPacketSendTrigger == 0)
			{
				if(g_Config.DebugPacketSendMessage == true)
					g_Log.AddMessageFormat("[SOCKET] Socket:%d failing to send (%d/%d), %d bytes remaining in %d packets", socketData.mSocket, sendRes, size, socketData.DebugGetPendingSize(), socketData.mPacketList.size());

				if(g_Config.DebugPacketSendDelay != 0)
				{
					mCountForceDelay++;
					socketData.mForcedResumeTime = g_ServerTime + g_Config.DebugPacketSendDelay;
				}
			}
			//End debugging.
...
int PacketManager :: AttemptSend2(int socket, const char *buffer, int length)
{
	...
	while(written < length)
	{
		...
		if(r == -1)
		{
			...
			//important.
			g_Log.AddMessageFormat("[SOCKET] Unspecified error: %d", mLastError);
			return -1;
		}
		written += r;
