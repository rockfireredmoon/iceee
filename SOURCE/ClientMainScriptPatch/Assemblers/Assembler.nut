...
{ 
	...
	/**
		Refresh (i.e. reconstruct/reassemble) a SceneObject that
		has been registered as using this assembler. A subclass
		should NOT override this, but _assemble() to provide useful
		construction of the visual look of an object.
		<p>
		Any data that needs to be retained on the scene object
		should be stored in the SceneObject's mAssemblyData field
		which is reserved for exactly this purpose.
		<p>
		Because this routine can be called at any time, it should
		only do useful work if it has enough information to do so.
		For instance, when still fetching or loading a definition
		from the server, a Scenery assembler cannot actually 
		assemble anything until that info has arrived. It's expected
		that the assembler bail out early (doing nothing) if it
		doesn't have enough information to continue (yet).
		
		@return True if the assembly completed successfully, or
			false if it wasn't ready yet (or some other problem).
	*/	
	function assemble(obj)
	{
		if( obj.isAssembled() )
			...
			//throw Exception("Cannot assemble duplicate of: " + obj);
			
		// Catching an error here unfortunately doesn't help us
		// much. It basically winds up causing more errors later
		// on that just become difficult to find. By not catching
		// it at least we can get SQDev to catch it while debugging.
		
		//if( /*("isRemoteDebugging" in ::System) && ::System.isRemoteDebugging()*/ false )
		//{
			// Leave exceptions unhandled for testing.
			return _assemble(obj);
		//}
		//else
		//{
//			// Use try/catch to ensure things continue loading.
			//try
			//{
				//return _assemble(obj);
			//}
			//catch( err )
			//{
				// We encountered an error during the assembly process.
				// We'll try to disassemble the object and setup a
				// an error node (A question-mark mesh) so we know
				// something happened in the client.
				
				//try
				//{
					//log.error("Error assembling " + obj + ": " + err);
					//mRequiredArchivesError = err;
					
					//onError( obj, err );
					//disassemble( obj );
					
					//createErrorNode( obj.getNode( ) );
				//}
				//catch ( err )
				//{
					//log.error( err );
