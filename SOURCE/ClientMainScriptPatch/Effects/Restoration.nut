...
class this.EffectDef.HealingScreamWarmup extends this.EffectDef.TemplateWarmup
{
	static mEffectName = "HealingScreamWarmup";
	mLoopSound = "Sound-Ability-Frostspells_Warmup.ogg";
	function onStart( ... )
	{
		if (!this._sourceCheck())
		...
		local warmup = this.createGroup("Warmup", this.getSource());
		warmup.add("ParticleSystem", {
			particleSystem = "Par-Healing_Warmup_FillRed",
			emitterPoint = "casting"
		});
		warmup.add("ParticleSystem", {
			particleSystem = "Par-Healing_Warmup_BloodDrops",
			emitterPoint = "casting"
		});
		warmup.add("ParticleSystem", {
			particleSystem = "Par-Healing_Warmup_Lines",
...
class this.EffectDef.HealingScream extends this.EffectDef.TemplateBasic
{
	static mEffectName = "HealingScream";
	mSound = "Sound-Ability-Default_Cast.ogg";
	function onStart( ... )
	{
		if (!this._targetCheck())
	...
	{
		this.get("Particles").finish();
		this.getTarget().cue("HealingScreamTarget", this.getTarget());
		local sparks = this.createGroup("Sparks", this.getSource());
		sparks.add("ParticleSystem", {
			particleSystem = "Par-Healing-Burst2",
			emitterPoint = "right_hand",
...
class this.EffectDef.HealingScreamTarget extends this.EffectDef.TemplateBasic
{
	static mEffectName = "HealingScreamTarget";
	mSound = "Sound-Ability-Healingscream_Effect.ogg";
	static mParticleLife = 0.69999999;
	static mSpinSpeed = 1.2;
	function onStart( ... )
	{
		...
		this.onSound();
		local heal = this.createGroup("RedEnergy", this.getSource());
		heal.add("ParticleSystem", {
			particleSystem = "Par-Healing-ImbueRed",
			emitterPoint = "core",
			particleScale = this.getSource().getScale().y,
			particleScaleProps = this.PSystemFlags.SIZE | this.PSystemFlags.VELOCITY
		});
