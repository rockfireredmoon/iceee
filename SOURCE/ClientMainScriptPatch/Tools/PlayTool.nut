...
class this.PlayTool extends this.AbstractCameraTool
{
	...
	function getCursor()
	{
		if (!this.mRotating && !this.mMoving && !this.mOrienting)
		{
			...
			if (so && so.isCreature())
			{
				...
				{
					if (so.getMeta("copper_shopkeeper") || so.getMeta("credit_shopkeeper") || so.getMeta("vault") || so.getMeta("auctioneer"))
					{
	...
	function onMouseReleased( evt )
	{
		...
		if (this.mDragDist < 4)
		{
			...
			else if (so == null)
			{
				...
				::_Connection.sendMouseClick(pointClickedOn.x, pointClickedOn.y, pointClickedOn.z);
				*/
			}
			else
			{		
	...
	function _selectNextCreature()
	{
		...
		{
			if (so.isInRange() && ::_avatar != so && !so.isPropCreature() && !so.isDead() && !so.getMeta("quest_giver") && !so.getMeta("quest_ender") && !so.getMeta("copper_shopkeeper") && !so.getMeta("credit_shopkeeper") && !so.getMeta("vault") && !so.getMeta("auctioneer") && so.getMeta("credit_shop") == null && !so.isPlayer() && !so.hasStatusEffect(this.StatusEffects.UNATTACKABLE) && !so.hasStatusEffect(this.StatusEffects.INVINCIBLE))
			{
