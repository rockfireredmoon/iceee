...
class this.GUI.QuestObjectives extends this.GUI.Component
{
	...
	constructor( ... )
	{
		...
		this.setSize(this.C_BASE_WIDTH, this.C_BASE_HEIGHT + (this.C_OBJECTIVE_HEIGHT * gMaxObjectives));
		this.setPreferredSize(this.C_BASE_WIDTH, this.C_BASE_HEIGHT + (this.C_OBJECTIVE_HEIGHT * gMaxObjectives));
...
class this.GUI.QuestExpGold extends this.GUI.Component
{
	...
	constructor( ... )
	{
		...
		this.mValourLabel = this.GUI.Label("Valour: ");
		this.mCostCurr = this.GUI.Currency();
		this.add(this.mExperienceLabel);
		this.add(this.mCostCurr);
		this.add(this.mValourLabel);
	}

	function updateExperience( exp )
	{
		this.mExperienceLabel.setText(exp == 0 ? "" : ( "Experience: " + exp));
	}

	function updateValour( valour )
	{
		this.mValourLabel.setText(valour == 0 ? "" : ( "Valour: " + valour ) );
	}
