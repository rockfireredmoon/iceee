require("GUI/LayoutManager");

/**
	Layout a component using the "border regions". These are defined to be
	the following:
	<pre>
	+------------------------------------------------+
	|                    North                       |
...
*/
class GUI.BorderLayout extends GUI.LayoutManager
{
	// Overridden, See LayoutManager#preferredLayoutSize
	function preferredLayoutSize(container)	{
		local bc = _getBorderTable(container);
		local sz = {
			width = 0,
			height = 0
		};
		...
		sz.width = Math.max(sz.width, topW);
		sz.width = Math.max(sz.width, botW);
		return container._addInsets(sz);
	}

	// Overridden, See LayoutManager#layoutContainer
	...
	/**
		Build a lookup table of the "north", "east", etc
		components and exclude everything else.
	*/
	function _getBorderTable( container ) {
		...
		foreach( c in container.components ) {
			...
			local constraint = getConstraint(container, c);
			...
			if (constraint == "north" || constraint == "south" || constraint == "east" || constraint == "west" || constraint == "center") {
				if (!(constraint in results)) {
					...
					print("[WARNING] multiple \"" + constraint + "\" components in " + container);
					results[constraint] = c;
				}
			}
		}
