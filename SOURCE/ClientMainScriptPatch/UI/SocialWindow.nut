...
/* 
 * Component for displaying details about a guild.
 */
class this.GUI.GuildListObject extends this.GUI.Component
{
	constructor( title, motto, rankTitle, rankLevel, valour, type, menu)
	{
		this.GUI.Component.constructor(this.GUI.BoxLayoutV());
		this.getLayoutManager().setExpand(true);
		this.setInsets(0, 5, -1, 5);
	...
	function _buildDisplayString()
	{
		local color = this.mType == 1 ? "ffffff" : "d2d2d2";
		this.mNameLabel.setText("<b><font color=\"" + color + 
			"\" size=\"16\">" + this.mTitle + "</font>  " + 
			(this.mType == 1 ? "<font size=\"16\" color=\"8aff87\">(Neutral)</font></b>" : "<font size=\"16\" color=\"d2d2d2\">(Faction)</font>"));
		local elementFont = "<font color=\"fff77f\">";
		local info = this.mMotto + "<br>" + elementFont + "Rank:</font> " + this.mRankTitle + " (" + this.mRankLevel + ")";
		this.mStatusLabel.setText("Valour: " + this.mValour);
		this.mInfoHTML.setText(info);
	}
	...
	function onMousePressed( evt )
	{
		if (this.mMenu != null && evt.button == this.MouseEvent.RBUTTON)
		{
			this.mMenu.showMenu();
		}
	}
...
class this.Screens.SocialWindow extends this.GUI.Frame
{
	...
	mGuildMemberActions = null;
	mGuildHeaderLabel = null;
	...
	constructor()
	{
		...
		this.mGuildHeaderLabel = this.GUI.HTML("");
		this.mGuildHeaderLabel.setPreferredSize(100, 20);
		this.mGuildHeaderLabel.setText("<i>Downloading guild information...</i>");
		this.mGuildComponent = this.GUI.Component(this.GUI.BorderLayout());
		local leaveGuildButton = this.GUI.Button("Leave Guild", this, "onLeaveGuild");
		...
		::_Connection.sendQuery("guild.info", this, []);
	...
	function onGuildLeaveAccept( window )
	{
		local guild = this.getSelectedGuild();
		window.destroy();
		::_Connection.sendQuery("guild.leave", this, [guild.defID]);
	}
	...
	function onLeaveGuild( button )
	{
		...
		if(guild) 
		{
			local request = ::GUI.PopupQuestionBox("Are you sure you want to leave the Guild \'" + guild.name + "\'?");
	...
	{
		local rows = this.mGuildBrowser.getSelectedRows();
		...
		if (index >= this.mGuilds.len())
		{
			return null;
		}

		return this.mGuilds[index];
	}
	...
	function updateGuildBrowser()
	{
		...
		foreach( f in this.mGuilds )
		{
			local obj = this.GUI.GuildListObject(f.name, f.motto, f.rankTitle, f.rankLevel, f.valour, f.guildType, null);
			this.mGuildBrowser.addRow([
				obj
			]);
			if (guild != null && guild.name == f.name)
			{
	...
	function onQueryComplete( qa, results )
	{
		...
		{		
			...
			{
				local questId = r[0];
				::_questManager._removeQuestData(questId.tointeger());
			}
	...
	function updateGuildPanel()
	{
		...
		if (this.mGuilds.len() == 0)
		{
			this.mGuildHeaderLabel.setText("<i>You are not in any guilds.</i>");
			this.mGuildComponent.add(this.mGuildHeaderLabel, this.GUI.BorderLayout.CENTER);
			this.mGuildMemberActions.setVisible(false);
		}
		else
		{
			this.mGuildHeaderLabel.setText("<i>You are in " + this.mGuilds.len() + " guilds.</i>");
			this.mGuildComponent.add(this.mGuildHeaderLabel, this.GUI.BorderLayout.NORTH);
	...
	function onGuildChange( guildDefID )
	{
		print("Guild change " + guildDefID);
		::_Connection.sendQuery("guild.info", this, []);
		//this.updateGuildBrowser();
		//this.updateGuildPanel();
	}
