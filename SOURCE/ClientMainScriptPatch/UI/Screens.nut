...
this.Screens.show <- function ( className, ... )
{
	...
	/* Keep Screen within current window */  	
	local pos = s.getPosition();
	if(pos.x + s.getWidth() > ::Screen.getWidth())
		pos.x = ::Screen.getWidth() - s.getWidth(); 
	if(pos.y + s.getHeight() > ::Screen.getHeight())
		pos.y = ::Screen.getHeight() - s.getHeight();
	if(pos.x < 0)
		pos.x = 0;
	if(pos.y < 0)
		pos.y = 0;
