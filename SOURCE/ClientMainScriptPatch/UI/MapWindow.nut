...
class this.Screens.MapWindow extends this.GUI.MainScreenElement
{
	...
	constructor()
	{
		...
		{ 
			...
			//print("ICE! MapWindow.construct " + this.mTargetZone + "\n");
			this._fetchList();
	...
	function update()
	{
		if (::_avatar)
		{
			...
			//print("ICE! MapWindow.update() " + position);
	...
	function _addMarker( markerObj )
	{
		...
		//print("ICE! MapWindow._addMarker onmap = " + isMarkerOnMap + " : " + itemType + " : " + markerObj.position.x + "," + markerObj.position.z + "\n");  
		...
		// DEBUG
		//print("ICE! temp set legend shown");
		isLegendShown = true;
	...
	function _fetchList()
	{
		if (this.mTargetZone != null)
		{
			...
			//print("ICE! MapWindow._fetchList " + this.mTargetZone + "\n");
	...
	function updatePartyMarkers()
	{
		...
		//print("ICE! MapWindow.updatePartyMembers " + partyMembers.len() + "\n");
	...
	function onExitFrame()
	{
		...
		if (!this.mInitializeMarkerList)
		{
			local position = ::_avatar.getNode().getPosition();
			this.mTargetZone = ::_sceneObjectManager.getCurrentZoneDefID();
			//print("ICE! MapWindow.onExitFrame mInitializeMarkerList " + this.mTargetZone + " @ " + position.x + " , " + position.z + "\n");
			this.mMapImage.setPlayerPosition(position);
			this._fetchList();
			this.mInitializeMarkerList = true;
		...
		{
			//print("ICE! MapWindow.onExitFrame " + playerPosition + "\n");
			...
			//print("ICE! MapWindow.onExitFrame player moved " + totalDistance + "\n");

			if (totalDistance >= this.MINIMUM_DISTANCE_MOVED)
			{
				//print("ICE! MapWindow.onExitFrame MOVED\n");
				this.mPlayerLastPosition = playerPosition;
				this.mPlayerMoved = true;
			}
			
			//if(this.mDistanceTimer) {				
				//print("ICE! MapWindow.onExitFrame UDM " + updateDistanceMarker + "DT " + this.mDistanceTimer.getMilliseconds() + "\n");
			//}
		}
