...
class this.IGISManager extends this.MessageBroadcaster
{
	...
	function addTransitory( message, type )
	{
		...
		if(type == this.IGIS.ERROR) 
		{
			label.setFontColor(this.Color(1.0, 0.0, 0.0, 0.0));
		}
		else if(type == this.IGIS.BROADCAST)
		{
			label.setFontColor(this.Color(0.2, 1.0, 0.2, 0.0));
		}
		else
		{
			label.setFontColor(this.Color(1.0, 1.0, 0.0, 0.0));
		} 
...
this.IGIS.broadcast <- function ( text )
{
	this.log.info(text);
