...
function PlaySound(sArchive, sName)
{
//	IGIS.info("Attempting to play:" + sArchive + ":" + sName);
	local callback =
	{
		archive = sArchive,
		name = sName,
		...
		{
//			IGIS.info("Delayed:" + name + ":" + info);
			_audioManager.playSound(name, "TEST");
		},
		function onPackageComplete(pkg)
		{
//			IGIS.info("Package complete:" + pkg);
//			IGIS.info("archive:" + archive);
//			IGIS.info("name:" + name);
			_audioManager.playSound(name, "TEST");
		}
		function onPackageError(pkg, error)
		{
//			IGIS.error("Error: package not found: " + pkg);
//			IGIS.error(error);
		}
	};
	local mLoader = (sArchive != "") ? Util.waitForAssets( sArchive, callback) : null;

	// If we have a loader, then we'll play then sound
	// when it has finished loading. Otherwise we'll
	// play the sound immediately.
	if ( mLoader == null )
	{
		_audioManager.playSound(sName, "TEST");
//		IGIS.info("Played:" + sName);
	}
	else
	{
//		IGIS.info("Ignored");
