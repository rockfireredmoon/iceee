...
class this.QuestManager extends this.DefaultQueryHandler
{
	...
	function _handleQuestDataReceiveV2( row )
	{
		...
		if (!(questId in this.mQuests))
		{
			questData = ::QuestData(questId, title, bodyText, "", objectives, rewards, numRewards, false, level, partySize, experience, coin, questEnderMarkerInfo);
			this.mQuests[questId] <- questData;
		}
		else
		{
	...
	function _handleQuestDataReceiveV3( row )
	{
		...
		if (!(questId in this.mQuests))
		{
			questData = ::QuestData(questId, title, bodyText, "", objectives, rewards, numRewards, unabandonable, level, partySize, experience, coin, questEnderMarkerInfo);
			this.mQuests[questId] <- questData;
		}
		else
		{
	...
	function _handleGenericQuestDataReceiveV2( row )
	{
		...
		if (!(questId in this.mGenericQuests))
		{
			questData = ::QuestData(questId, title, bodyText, completionText, objectives, rewards, numRewards, false, level, partySize, experience, coin, questEnderMarkerInfo);
			this.mGenericQuests[questId] <- questData;
		}
		else
		{
	...
	function _handleGenericQuestDataReceiveV3( row )
	{
		...
		if (!(questId in this.mGenericQuests))
		{
			questData = ::QuestData(questId, title, bodyText, completionText, objectives, rewards, numRewards, unabandonable, level, partySize, experience, coin, questEnderMarkerInfo);
			this.mGenericQuests[questId] <- questData;
		}
		else
		{
...
class this.QuestData 
{
	...
	constructor( id, title, body, completionBody, objectives, rewardChoices, numRewards, unabandonable, suggestedLevel, partySize, experience, currencyGained, questEnderMarkerInfo )
	{
		this.mObjectives = [];
		this.mRewardChoices = [];
