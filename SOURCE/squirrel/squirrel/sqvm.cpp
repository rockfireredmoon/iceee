...
void SQVM::CallDebugHook(SQInteger sqtype,SQInteger forcedline)
{
    _debughook = false;
    ...
    if(_debughook_native) {
        ...
        _debughook_native(this,sqtype,src,line,fname);
    }
    else {
        SQObjectPtr temp_reg;
        SQInteger nparams=5;
        Push(_roottable); Push(sqtype); Push(func->_sourcename); Push(forcedline?forcedline:func->GetLine(ci->_ip)); Push(func->_name);
