...
static bool sq_aux_gettypedarg(HSQUIRRELVM v,SQInteger idx,SQObjectType sqtype,SQObjectPtr **o)
{
    *o = &stack_get(v,idx);
    if(sqtype(**o) != sqtype){
        SQObjectPtr oval = v->PrintObjVal(**o);
        v->Raise_Error(_SC("wrong argument type, expected '%s' got '%.50s'"),IdType2Name(sqtype),_stringval(oval));
        return false;
    }
    return true;
}
...
SQInteger sq_aux_invalidtype(HSQUIRRELVM v,SQObjectType sqtype)
{
    SQUnsignedInteger buf_size = 100 *sizeof(SQChar);
    scsprintf(_ss(v)->GetScratchPad(buf_size), buf_size, _SC("unexpected type %s"), IdType2Name(sqtype));
...
SQInteger sq_getsize(HSQUIRRELVM v, SQInteger idx)
{
    ...
    default:
        return sq_aux_invalidtype(v, sqtype);
    }
}
...
SQRESULT sq_setdelegate(HSQUIRRELVM v,SQInteger idx)
{
    ...
    SQObjectType sqtype = sqtype(self);
    switch(sqtype) {
    case OT_TABLE:
        ...
        else return sq_aux_invalidtype(v,sqtype);
        break;
    case OT_USERDATA:
        if(sqtype(mt)==OT_TABLE) {
